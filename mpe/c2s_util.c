...
/****
     ...
     KARSTEN:
     This function now runs in 2 phases. 
     Phase 0 handles statedefs only
     Phase 2 handles all raw events.
****/
int makeSLOG(double *membuff,int pass) {
  int  rec_type;
  CLOG_HEADER* headr;
  CLOG_STATE* state;          /* pointer to a clog state definition.*/ 
  ...
  while((rec_type != CLOG_ENDBLOCK) && (rec_type != CLOG_ENDLOG)) {
    ...
    case CLOG_RAWEVENT:
      ...
      adjust_CLOG_RAW ((CLOG_RAW *)data_ptr);
#endif
      if(pass==1 && logEvent( headr, (CLOG_RAW *)data_ptr) == C2S_ERROR)
	return C2S_ERROR;
      data_ptr = (double *) (((CLOG_RAW *) data_ptr)->end);
      break;
...
#endif
      state = (CLOG_STATE *)data_ptr;
      if( pass == 0 && handle_extra_state_defs(state) == C2S_ERROR )
	return C2S_ERROR;
      data_ptr = (double *) (((CLOG_STATE *) data_ptr)->end);
      break;
  ...
  /* Now all state definitions are known.
     We can now handle all other events. */
  if(pass==0) makeSLOG(membuff,1);
  return rec_type;
}
...
int logEvent( CLOG_HEADER *headr, CLOG_RAW *event_ptr ) {
  ...
  if((stat_id = findState_strtEvnt(event_ptr->etype)) != C2S_ERROR) {
    ...
    */
    if(stat_id == MSG_STATE)
      found_matching_msg = 
        find_msg_elemnt(stat_id, event_ptr->data, headr->procid,
                        event_ptr->etype, &one);
    if(found_matching_msg == C2S_ERROR)
    ...
    */
    ...
    /*one = (struct list_elemnt *) (MALLOC (sizeof(struct list_elemnt)));*/
    
    if(find_elemnt(stat_id,event_ptr->data,headr->procid, &one) == C2S_ERROR) {
	...
	fprintf(stderr, __FILE__":%d: couldnt find matching start event"
		",state=%d,etype=%d,processid=%d,data=%d,timestamp=%f\n", __LINE__,
		stat_id, event_ptr->etype,headr->procid, event_ptr->data, headr->timestamp );
	/*
	printEventList();
	printStateInfo();
  ...
  /* something i had thought would be useful but it isnt....
     actually this else could be used to log the sends and recieves for
     arrows in jumpshot.
  else {
   
    fprintf(stderr, __FILE__":%d: A missing state definition in the clog file"
	    "%d\n",__LINE__,event_ptr->etype);
...
int handle_extra_state_defs(CLOG_STATE *state) {
  ...
  */
  ...
  ierr = SLOG_PROF_AddExtraIntvlInfo( slog, intvltype, CLASS_TYPE,
...
int writeSLOGInterval(CLOG_HEADER *headr, CLOG_RAW *event_ptr,
  ...
  /*
    SLOG STUFF:
    we are not interested in bebits, cpu_id, thread_id, irec_where(i am not too
    sure about that one), and the rec_type is always 1 - it represents a fixed
...
int init_SLOG_PROF_RECDEF() {
  ...
  for( one = first ; one != NULL ; one = two ) {
    ...
    if((one->start_event_num != LOG_MESG_SEND) ||
      ...
      ierr = SLOG_PROF_AddIntvlInfo( slog, intvltype, CLASS_TYPE,
				     one->description, one->color);
    else {
      ierr = SLOG_PROF_AddIntvlInfo( slog, FORWARD_MSG,
	...
	return C2S_ERROR;
      ...
      ierr = SLOG_PROF_AddIntvlInfo( slog, BACKWARD_MSG,
...
void printHelp() {
    fprintf(stdout,"Usage : clog2slog [ -d=FrameNum ] [ -f=FrameSize ]"
	    ...
	    "\th : help menu\n\n"
            ...
            "guess shown in the error message of previous run of clog2slog.\n"
            "e.g. clog2slog -d=NewFrameNumber filename.clog\n"
