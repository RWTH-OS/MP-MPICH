...
#ifdef WIN32
char   *CLOG_tmpfilename=NULL;    /* temp log file name (abhi) */ 
#else
...
void CLOG_temp_log( void )
{
    ...
    if(CLOG_tempFD == -5) {
#ifdef WIN32
		CLOG_tempFD = open(CLOG_tmpfilename, O_RDWR|O_CREAT|O_TRUNC|O_BINARY, 0600);
#else
...
#else
	CLOG_tmpfilename = tempnam(NULL,"temp_clog");
#endif
}
