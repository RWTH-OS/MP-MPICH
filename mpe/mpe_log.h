...
IMPORT_MPI_API int MPE_Init_log _ANSI_ARGS_((void));
  /* call before calling any other logging functions */

IMPORT_MPI_API int MPE_Initialized_logging _ANSI_ARGS_ ((void));

IMPORT_MPI_API int MPE_Describe_state _ANSI_ARGS_(( int start, int end, char *name,
				   char *color ));
  /* create state description lines */

IMPORT_MPI_API int MPE_Describe_event _ANSI_ARGS_(( int event, char *name));
  /* create event description lines */

IMPORT_MPI_API int MPE_Log_get_event_number _ANSI_ARGS_(( void ));
  /* Get a new event number */
IMPORT_MPI_API int MPE_Start_log _ANSI_ARGS_((void));
  /* set timer to 0 */

IMPORT_MPI_API int MPE_Log_event _ANSI_ARGS_(( int event, int data, char *string ));
  /* log one event */

IMPORT_MPI_API int MPE_Log_send _ANSI_ARGS_(( int receiver, int tag, int size ));
  /* log the sending of a message */

IMPORT_MPI_API int MPE_Log_receive _ANSI_ARGS_(( int sender, int tag, int size ));
  /* log the receiving of a message */

IMPORT_MPI_API int MPE_Stop_log _ANSI_ARGS_((void));
  /* stop logging events */

IMPORT_MPI_API int MPE_Finish_log _ANSI_ARGS_(( char *filename ));
