...
MPI_Init( &argc, &argv );
  ...
  if (   (IsArgPresent( &argc, argv, "-h" ))
	  || (IsArgPresent( &argc, argv, "-help" ))
	  || (IsArgPresent( &argc, argv, "-?" ))) {
    PrintHelp( argv[0] );
  } 
#endif
...
*/
  ...
  if (allWindowsOpened) {
	...
	if (IsArgPresent( &argc, argv, "-movie" )) {
	    ...
	    CaptureFile = "mandel_out";
	    GetStringArg(&argc,argv,"-capture",&CaptureFile);
	    MPE_CaptureFile( graph, CaptureFile, freq );
	    }
	}
    if (!winspecs.bw) {
...
Flags *flags;
{
    int x1, y1, x2, y2, i, myid, button,res;
    NUM zx1, zy1, zx2, zy2;
	...
	res = MPE_Iget_mouse_press( graph, &x1, &y1, &button, &i );
	if (res != MPE_SUCCESS || (i && button == MPE_BUTTON2)) {
	    flags->zoom = 0;
	} else {
	    res=MPE_Get_drag_region( graph, 1, MPE_DRAG_SQUARE, &x1, &y1, &x2, &y2 );
	    if(res == MPE_SUCCESS) {
