...
int FORTRAN_API mpe_init_log_ ANSI_ARGS(( void ));
int FORTRAN_API mpe_start_log_ ANSI_ARGS(( void ));
int FORTRAN_API mpe_stop_log_ ANSI_ARGS(( void ));
...
int FORTRAN_API mpe_init_log_()
{
    return MPE_Init_log();
}
int FORTRAN_API mpe_start_log_()
{
    return MPE_Start_log();
}
int FORTRAN_API mpe_stop_log_()
{
    return MPE_Stop_log();
}

int FORTRAN_API mpe_log_get_event_number_()
{
...
int FORTRAN_API mpe_describe_state_ ANSI_ARGS(( int *, int *, char *, int ,char *, int ));
int  FORTRAN_API mpe_describe_state_( start, end, name,d1, color, d2 )
int *start, *end;
char *name;
int  d1;
char* color;
...
#else
int FORTRAN_API mpe_describe_state_ ANSI_ARGS(( int *, int *, char *, char *, int, int ));
int  FORTRAN_API mpe_describe_state_( start, end, name, color, d1, d2 )
int *start, *end;
char *name, *color;
int  d1, d2;
{
...
#else
int FORTRAN_API mpe_describe_event_ ANSI_ARGS(( int *, char *, int ));
int  FORTRAN_API mpe_describe_event_( event, name, d1)
int *event;
char *name;
int  d1;
{
...
#else
int FORTRAN_API mpe_log_event_ ANSI_ARGS(( int *, int *, char *, int ));
int  FORTRAN_API mpe_log_event_(event,data,string, d1)
int *event, *data;
char *string;
int  d1;
{
...
#else
int FORTRAN_API mpe_finish_log_ ANSI_ARGS(( char *, int ));
int FORTRAN_API mpe_finish_log_( filename, d1)
char *filename;
int  d1;
{
    char *c1;
