...
/*@
    NEW: MPE_Fill_polygon - Draws a filled polygon on an X11 display 

    Input Parameters:
+   handle - MPE graphics handle 
.   points - list of the polygon-points
...
-   color - Color `index` value.  See 'MPE_MakeColorArray'.  
    ...
    Notes:
    color-values of the MPE_Point-struct are not used
    
@*/
int MPE_Fill_polygon( handle, points, npoints, color )
MPE_XGraph handle;
MPE_Point  *points;
int        npoints;
MPE_Color  color;
{ 
  ...
  {        
    for(i=0; (i<npoints) && (i<256); i++) {
      xpoints[i].x = points[i].x;  xpoints[i].y = points[i].y;
    }

    XBSetPixVal( handle->xwin, handle->xwin->cmapping[color] );
    XFillPolygon( handle->xwin->disp, handle->xwin->win, handle->xwin->gc.set, 
...
/*@
   NEW: MPE_Draw_Sstring - Draw a sized text string
...
- size - font size
@*/
int MPE_Draw_font( graph, x, y, color, string, name )
MPE_XGraph graph;
int x, y;
MPE_Color color;
char *string;
char *name;
{
  ...
  myFont=XLoadFont(graph->xwin->disp, name);
  XSetFont(graph->xwin->disp, graph->xwin->gc.set, myFont);
...
MPE_Color *mapping;
{
  ...
  */
  if (graph->xwin->maxcolors == 655366)
    graph->xwin->maxcolors = 65535;
  return MPE_SUCCESS;
}
