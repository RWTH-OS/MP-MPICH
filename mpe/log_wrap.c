...
int _mpe_trace_on = 0;
MPE_State _mpe_states[MPE_MAX_STATES];
...
*/
  ...
  /* First, get the total number of calls by any processor */
	 ...
	 for (i=0; i<MPE_MAX_STATES; i++) {
	     if (totcnt[i] > 0) {
		 MPE_Describe_state( _mpe_states[i].id, _mpe_states[i].id + 1, 
				     _mpe_states[i].name, _mpe_states[i].color );
	     }
	 }
     }
     ...
     if (procid_0 == 0) {
	 logfilename = (char*)malloc( strlen(logFileName_0) + 6 );
	 strcpy(logfilename, logFileName_0);
	 /* copied from mpe_log.c */
	 env_log_format = (char *)getenv("MPE_LOG_FORMAT");
	 if ( env_log_format != NULL ) {
	     if (strcmp(env_log_format,"ALOG") == 0) strcat(logfilename, ".alog");
	     else if (strcmp(env_log_format,"SLOG") == 0) strcat(logfilename, ".slog");
	     /* unrecognized logformat. .clog taken */
	     else strcat(logfilename, ".clog");
	 }
	 else strcat(logfilename, ".clog");
...
char *** argv;
{
  ...
  for (i=0; i<MPE_MAX_STATES; i++) {
  	_mpe_states[i].n_calls   = 0;
  	_mpe_states[i].id	    = stateid; stateid += 2;
  	_mpe_states[i].is_active = 0;
  	_mpe_states[i].name	    = 0;
  	_mpe_states[i].kind_mask = 0;
  	_mpe_states[i].color	    = "white:vlines";
  }

  /* By default, log only message-passing (pt-to-pt, collective and io) */
  allow_mask = MPE_KIND_MSG | MPE_KIND_COLL | MPE_KIND_FILE | MPE_KIND_ADIO;
  ...
  */
  state = &_mpe_states[MPE_ALLGATHER_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "ALLGATHER";

  state = &_mpe_states[MPE_ALLGATHERV_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "ALLGATHERV";

  state = &_mpe_states[MPE_ALLREDUCE_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "ALLREDUCE";
  state->color = "purple:vlines3";

  state = &_mpe_states[MPE_ALLTOALL_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "ALLTOALL";

  state = &_mpe_states[MPE_ALLTOALLV_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "ALLTOALLV";

  state = &_mpe_states[MPE_BARRIER_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "BARRIER";
  state->color = "yellow:dimple3";

  state = &_mpe_states[MPE_BCAST_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "BCAST";
  state->color = "cyan:boxes";

  state = &_mpe_states[MPE_GATHER_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "GATHER";

  state = &_mpe_states[MPE_OP_CREATE_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "OP_CREATE";

  state = &_mpe_states[MPE_OP_FREE_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "OP_FREE";

  state = &_mpe_states[MPE_REDUCE_SCATTER_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "REDUCE_SCATTER";

  state = &_mpe_states[MPE_REDUCE_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "REDUCE";
  state->color = "purple:2x2";

  state = &_mpe_states[MPE_SCAN_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "SCAN";

  state = &_mpe_states[MPE_SCATTER_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "SCATTER";

  state = &_mpe_states[MPE_SCATTERV_ID];
  state->kind_mask = MPE_KIND_COLL;
  state->name = "SCATTERV";

  state = &_mpe_states[MPE_ATTR_DELETE_ID];
  state->kind_mask = MPE_KIND_ATTR;
  state->name = "ATTR_DELETE";

  state = &_mpe_states[MPE_ATTR_GET_ID];
  state->kind_mask = MPE_KIND_ATTR;
  state->name = "ATTR_GET";

  state = &_mpe_states[MPE_ATTR_PUT_ID];
  state->kind_mask = MPE_KIND_ATTR;
  state->name = "ATTR_PUT";

  state = &_mpe_states[MPE_COMM_COMPARE_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_COMPARE";

  state = &_mpe_states[MPE_COMM_CREATE_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "COMM_CREATE";

  state = &_mpe_states[MPE_COMM_DUP_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "COMM_DUP";

  state = &_mpe_states[MPE_COMM_FREE_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "COMM_FREE";

  state = &_mpe_states[MPE_COMM_GROUP_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_GROUP";

  state = &_mpe_states[MPE_COMM_RANK_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_RANK";

  state = &_mpe_states[MPE_COMM_REMOTE_GROUP_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_REMOTE_GROUP";

  state = &_mpe_states[MPE_COMM_REMOTE_SIZE_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_REMOTE_SIZE";

  state = &_mpe_states[MPE_COMM_SIZE_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_SIZE";

  state = &_mpe_states[MPE_COMM_SPLIT_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "COMM_SPLIT";

  state = &_mpe_states[MPE_COMM_TEST_INTER_ID];
  state->kind_mask = MPE_KIND_COMM_INFO;
  state->name = "COMM_TEST_INTER";

  state = &_mpe_states[MPE_GROUP_COMPARE_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_COMPARE";

  state = &_mpe_states[MPE_GROUP_DIFFERENCE_ID];  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_DIFFERENCE";

  state = &_mpe_states[MPE_GROUP_EXCL_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_EXCL";

  state = &_mpe_states[MPE_GROUP_FREE_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_FREE";

  state = &_mpe_states[MPE_GROUP_INCL_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_INCL";

  state = &_mpe_states[MPE_GROUP_INTERSECTION_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_INTERSECTION";

  state = &_mpe_states[MPE_GROUP_RANK_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_RANK";

  state = &_mpe_states[MPE_GROUP_RANGE_EXCL_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_RANGE_EXCL";

  state = &_mpe_states[MPE_GROUP_RANGE_INCL_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_RANGE_INCL";

  state = &_mpe_states[MPE_GROUP_SIZE_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_SIZE";

  state = &_mpe_states[MPE_GROUP_TRANSLATE_RANKS_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_TRANSLATE_RANKS";

  state = &_mpe_states[MPE_GROUP_UNION_ID];
  state->kind_mask = MPE_KIND_GROUP;
  state->name = "GROUP_UNION";

  state = &_mpe_states[MPE_INTERCOMM_CREATE_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "INTERCOMM_CREATE";

  state = &_mpe_states[MPE_INTERCOMM_MERGE_ID];
  state->kind_mask = MPE_KIND_COMM;
  state->name = "INTERCOMM_MERGE";

  state = &_mpe_states[MPE_KEYVAL_CREATE_ID];
  state->kind_mask = MPE_KIND_ATTR;
  state->name = "KEYVAL_CREATE";

  state = &_mpe_states[MPE_KEYVAL_FREE_ID];
  state->kind_mask = MPE_KIND_ATTR;
  state->name = "KEYVAL_FREE";

  state = &_mpe_states[MPE_ABORT_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ABORT";

  state = &_mpe_states[MPE_ERROR_CLASS_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERROR_CLASS";

  state = &_mpe_states[MPE_ERRHANDLER_CREATE_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERRHANDLER_CREATE";

  state = &_mpe_states[MPE_ERRHANDLER_FREE_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERRHANDLER_FREE";

  state = &_mpe_states[MPE_ERRHANDLER_GET_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERRHANDLER_GET";

  state = &_mpe_states[MPE_ERROR_STRING_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERROR_STRING";

  state = &_mpe_states[MPE_ERRHANDLER_SET_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "ERRHANDLER_SET";

  state = &_mpe_states[MPE_GET_PROCESSOR_NAME_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "GET_PROCESSOR_NAME";

  state = &_mpe_states[MPE_INITIALIZED_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "INITIALIZED";

  state = &_mpe_states[MPE_WTICK_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "WTICK";

  state = &_mpe_states[MPE_WTIME_ID];
  state->kind_mask = MPE_KIND_ENV;
  state->name = "WTIME";

  state = &_mpe_states[MPE_ADDRESS_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "ADDRESS";

  state = &_mpe_states[MPE_BSEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "BSEND";
  state->color = "blue:gray3";

  state = &_mpe_states[MPE_BSEND_INIT_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "BSEND_INIT";

  state = &_mpe_states[MPE_BUFFER_ATTACH_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "BUFFER_ATTACH";

  state = &_mpe_states[MPE_BUFFER_DETACH_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "BUFFER_DETACH";

  state = &_mpe_states[MPE_CANCEL_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "CANCEL";

  state = &_mpe_states[MPE_REQUEST_FREE_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "REQUEST_FREE";

  state = &_mpe_states[MPE_RECV_INIT_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "RECV_INIT";

  state = &_mpe_states[MPE_SEND_INIT_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "SEND_INIT";

  state = &_mpe_states[MPE_GET_ELEMENTS_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "GET_ELEMENTS";

  state = &_mpe_states[MPE_GET_COUNT_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "GET_COUNT";

  state = &_mpe_states[MPE_IBSEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "IBSEND";

  state = &_mpe_states[MPE_IPROBE_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "IPROBE";
  state->color = "seagreen:gray";

  state = &_mpe_states[MPE_IRECV_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "IRECV";
  state->color = "springgreen:gray";

  state = &_mpe_states[MPE_IRSEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "IRSEND";

  state = &_mpe_states[MPE_ISEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "ISEND";
  state->color = "skyblue:gray";
  
  state = &_mpe_states[MPE_ISSEND_ID];
  state->kind_mask = MPE_ISSEND_ID;
  state->name = "ISSEND";
  state->color = "seagreen:gray";
  
  state = &_mpe_states[MPE_PACK_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "PACK";
  
  state = &_mpe_states[MPE_PACK_SIZE_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "PACK_SIZE";

  state = &_mpe_states[MPE_PROBE_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "PROBE";
  state->color = "seagreen:gray";
  
  state = &_mpe_states[MPE_RECV_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "RECV";
  state->color = "green:light_gray";
  
  state = &_mpe_states[MPE_RSEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "RSEND";
  state->color = "DeepSkyBlue:gray";

  state = &_mpe_states[MPE_RSEND_INIT_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "RSEND_INIT";

  state = &_mpe_states[MPE_SEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "SEND";
  state->color = "blue:gray3";

  state = &_mpe_states[MPE_SENDRECV_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "SENDRECV";
  state->color = "seagreen:gray";
  
  state = &_mpe_states[MPE_SENDRECV_REPLACE_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "SENDRECV_REPLACE";
  state->color = "seagreen:gray";

  state = &_mpe_states[MPE_SSEND_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "SSEND";
  state->color = "deepskyblue:gray";

  state = &_mpe_states[MPE_SSEND_INIT_ID];
  state->kind_mask = MPE_KIND_MSG_INIT;
  state->name = "SSEND_INIT";

  state = &_mpe_states[MPE_START_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "START";

  state = &_mpe_states[MPE_STARTALL_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "STARTALL";
  
  state = &_mpe_states[MPE_TEST_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "TEST";
  state->color = "orange:gray";

  state = &_mpe_states[MPE_TESTALL_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "TESTALL";
  state->color = "orange:gray";

  state = &_mpe_states[MPE_TESTANY_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "TESTANY";
  state->color = "orange:gray";

  state = &_mpe_states[MPE_TEST_CANCELLED_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "TEST_CANCELLED";

  state = &_mpe_states[MPE_TESTSOME_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "TESTSOME";
  state->color = "orange:gray";

  state = &_mpe_states[MPE_TYPE_COMMIT_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_COMMIT";

  state = &_mpe_states[MPE_TYPE_CONTIGUOUS_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_CONTIGUOUS";

  state = &_mpe_states[MPE_TYPE_EXTENT_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_EXTENT";

  state = &_mpe_states[MPE_TYPE_FREE_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_FREE";

  state = &_mpe_states[MPE_TYPE_HINDEXED_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_HINDEXED";

  state = &_mpe_states[MPE_TYPE_INDEXED_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_INDEXED";

  state = &_mpe_states[MPE_TYPE_HVECTOR_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_HVECTOR";

  state = &_mpe_states[MPE_TYPE_LB_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_LB";

  state = &_mpe_states[MPE_TYPE_SIZE_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_SIZE";

  state = &_mpe_states[MPE_TYPE_STRUCT_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_STRUCT";

  state = &_mpe_states[MPE_TYPE_UB_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_UB";

  state = &_mpe_states[MPE_TYPE_VECTOR_ID];
  state->kind_mask = MPE_KIND_DATATYPE;
  state->name = "TYPE_VECTOR";

  state = &_mpe_states[MPE_UNPACK_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "UNPACK";

  state = &_mpe_states[MPE_WAIT_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "WAIT";
  state->color = "red:black";

  state = &_mpe_states[MPE_WAITALL_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "WAITALL";
  state->color = "OrangeRed:gray";

  state = &_mpe_states[MPE_WAITANY_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "WAITANY";
  state->color = "coral:gray";

  state = &_mpe_states[MPE_WAITSOME_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "WAITSOME";
  state->color = "red:black";
  
  state = &_mpe_states[MPE_CART_COORDS_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_COORDS";

  state = &_mpe_states[MPE_CART_CREATE_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_CREATE";

  state = &_mpe_states[MPE_CART_GET_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_GET";

  state = &_mpe_states[MPE_CART_MAP_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_MAP";

  state = &_mpe_states[MPE_CART_RANK_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_RANK";

  state = &_mpe_states[MPE_CART_SHIFT_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_SHIFT";

  state = &_mpe_states[MPE_CART_SUB_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CART_SUB";

  state = &_mpe_states[MPE_CARTDIM_GET_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "CARTDIM_GET";

  state = &_mpe_states[MPE_DIMS_CREATE_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "DIMS_CREATE";

  state = &_mpe_states[MPE_GRAPH_CREATE_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPH_CREATE";

  state = &_mpe_states[MPE_GRAPH_GET_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPH_GET";

  state = &_mpe_states[MPE_GRAPH_MAP_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPH_MAP";

  state = &_mpe_states[MPE_GRAPH_NEIGHBORS_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPH_NEIGHBORS";

  state = &_mpe_states[MPE_GRAPH_NEIGHBORS_COUNT_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPH_NEIGHBORS_COUNT";

  state = &_mpe_states[MPE_GRAPHDIMS_GET_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "GRAPHDIMS_GET";

  state = &_mpe_states[MPE_TOPO_TEST_ID];
  state->kind_mask = MPE_KIND_TOPO;
  state->name = "TOPO_TEST";

  state = &_mpe_states[MPE_RECV_IDLE_ID];
  state->kind_mask = MPE_KIND_MSG;
  state->name = "RECV_IDLE";

  /* states for MPI-IO */
  state = &_mpe_states[MPE_FILE_CALL_ERRHANDLER_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_CALL_ERRHANDLER";
  
  state = &_mpe_states[MPE_FILE_CLOSE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_CLOSE";
  state->color = "IndianRed:vlines1";

  state = &_mpe_states[MPE_FILE_CREATE_ERRHANDLER_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_CREATE_ERRHANDLER";

  state = &_mpe_states[MPE_FILE_DELETE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_DELETE";

  state = &_mpe_states[MPE_FILE_GET_AMODE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_AMODE";

  state = &_mpe_states[MPE_FILE_GET_ATOMICITY_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_ATOMICITY";

  state = &_mpe_states[MPE_FILE_GET_BYTE_OFFSET_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_BYTE_OFFSET";

  state = &_mpe_states[MPE_FILE_GET_ERRHANDLER_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_ERRHANDLER";

  state = &_mpe_states[MPE_FILE_GET_GROUP_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_GROUP";

  state = &_mpe_states[MPE_FILE_GET_INFO_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_INFO";

  state = &_mpe_states[MPE_FILE_GET_POSITION_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_POSITION";

  state = &_mpe_states[MPE_FILE_GET_POSITION_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_POSITION_SHARED";

  state = &_mpe_states[MPE_FILE_GET_SIZE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_SIZE";

  state = &_mpe_states[MPE_FILE_GET_TYPE_EXTENT_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_TYPE_EXTENT";

  state = &_mpe_states[MPE_FILE_GET_VIEW_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_GET_VIEW";

  state = &_mpe_states[MPE_FILE_IREAD_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IREAD";
  state->color = "SeaGreen1:vlines1";

  state = &_mpe_states[MPE_FILE_IREAD_AT_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IREAD_AT";
  state->color = "SeaGreen2:vlines1";

  state = &_mpe_states[MPE_FILE_IREAD_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IREAD_SHARED";
  state->color = "SeaGreen1:vlines1";

  state = &_mpe_states[MPE_FILE_IWRITE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IWRITE";
  state->color = "SteelBlue:vlines1";

  state = &_mpe_states[MPE_FILE_IWRITE_AT_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IWRITE_AT";
  state->color = "LightSteelBlue:vlines1";

  state = &_mpe_states[MPE_FILE_IWRITE_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_IWRITE_SHARED";
  state->color = "PowderBlue:vlines1";

  state = &_mpe_states[MPE_FILE_OPEN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_OPEN";
  state->color = "OrangeRed:vlines1";

  state = &_mpe_states[MPE_FILE_PREALLOCATE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_PREALLOCATE";
  state->color = "VioletRed:vlines1";

  state = &_mpe_states[MPE_FILE_READ_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ";
  state->color = "seagreen:vlines1";

  state = &_mpe_states[MPE_FILE_READ_ALL_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ALL";
  state->color = "MediumSeaGreen:vlines2";

  state = &_mpe_states[MPE_FILE_READ_ALL_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ALL_BEGIN";
  state->color = "LightSeaGreen:vlines3";

  state = &_mpe_states[MPE_FILE_READ_ALL_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ALL_END";
  state->color = "PaleGreen:vlines3";

  state = &_mpe_states[MPE_FILE_READ_AT_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_AT";
  state->color = "LawnGreen:vlines1";

  state = &_mpe_states[MPE_FILE_READ_AT_ALL_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_AT_ALL";
  state->color = "GreenYellow:vlines1";

  state = &_mpe_states[MPE_FILE_READ_AT_ALL_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_AT_ALL_BEGIN";
  state->color = "LimeGreen:vlines1";

  state = &_mpe_states[MPE_FILE_READ_AT_ALL_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_AT_ALL_END";
  state->color = "LimeGreen:vlines1";

  state = &_mpe_states[MPE_FILE_READ_ORDERED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ORDERED";

  state = &_mpe_states[MPE_FILE_READ_ORDERED_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ORDERED_BEGIN";

  state = &_mpe_states[MPE_FILE_READ_ORDERED_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_ORDERED_END";

  state = &_mpe_states[MPE_FILE_READ_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_READ_SHARED";
  state->color = "ForestGreen:vlines1";

  state = &_mpe_states[MPE_FILE_SEEK_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SEEK";
  state->color = "HotPink:vlines1";

  state = &_mpe_states[MPE_FILE_SEEK_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SEEK_SHARED";

  state = &_mpe_states[MPE_FILE_SET_ATOMICITY_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SET_ATOMICITY";

  state = &_mpe_states[MPE_FILE_SET_ERRHANDLER_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SET_ERRHANDLER";

  state = &_mpe_states[MPE_FILE_SET_INFO_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SET_INFO";

  state = &_mpe_states[MPE_FILE_SET_SIZE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SET_SIZE";

  state = &_mpe_states[MPE_FILE_SET_VIEW_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SET_VIEW";

  state = &_mpe_states[MPE_FILE_SYNC_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_SYNC";
  state->color = "beige:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE";
  state->color = "NavyBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ALL_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ALL";
  state->color = "MidnightBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ALL_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ALL_BEGIN";
  state->color = "AliceBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ALL_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ALL_END";
  state->color = "CornflowerBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_AT_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_AT";
  state->color = "DarkSlateBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_AT_ALL_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_AT_ALL";
  state->color = "MediumSlateBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_AT_ALL_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_AT_ALL_BEGIN";
  state->color = "LightSlateBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_AT_ALL_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_AT_ALL_END";
  state->color = "MediumBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ORDERED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ORDERED";
  state->color = "RoyalBlue:hlines1";

  state = &_mpe_states[MPE_FILE_WRITE_ORDERED_BEGIN_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ORDERED_BEGIN";

  state = &_mpe_states[MPE_FILE_WRITE_ORDERED_END_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_ORDERED_END";

  state = &_mpe_states[MPE_FILE_WRITE_SHARED_ID];
  state->kind_mask = MPE_KIND_FILE;
  state->name = "FILE_WRITE_SHARED";
  state->color = "DodgerBlue:hlines1";

  /* States for ADIO */
  state = &_mpe_states[MPE_PVFS_CLOSE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_CLOSE";

  state = &_mpe_states[MPE_PVFS_READDONE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_READDONE";

  state = &_mpe_states[MPE_PVFS_WRITEDONE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_WRITEDONE";

  state = &_mpe_states[MPE_PVFS_FCNTL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_FCNTL";

  state = &_mpe_states[MPE_PVFS_FLUSH_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_FLUSH";

  state = &_mpe_states[MPE_PVFS_SETINFO_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_SETINFO";

  state = &_mpe_states[MPE_PVFS_IREADCONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_IREADCONTIG";

  state = &_mpe_states[MPE_PVFS_IREADSTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_IREADSTRIDED";

  state = &_mpe_states[MPE_PVFS_IWRITECONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_IWRITECONTIG";

  state = &_mpe_states[MPE_PVFS_IWRITESTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_IWRITESTRIDED";

  state = &_mpe_states[MPE_PVFS_OPEN_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_OPEN";

  state = &_mpe_states[MPE_PVFS_READSTRIDEDCOLL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_READSTRIDEDCOLL";

  state = &_mpe_states[MPE_PVFS_READCONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_READCONTIG";

  state = &_mpe_states[MPE_PVFS_READSTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_READSTRIDED";

  state = &_mpe_states[MPE_PVFS_RESIZE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_RESIZE";

  state = &_mpe_states[MPE_PVFS_SEEKINDIVIDUAL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_SEEKINDIVIDUAL";

  state = &_mpe_states[MPE_PVFS_READCOMPLETE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_READCOMPLETE";

  state = &_mpe_states[MPE_PVFS_WRITECOMPLETE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_WRITECOMPLETE";

  state = &_mpe_states[MPE_PVFS_WRITESTRIDEDCOLL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_WRITESTRIDEDCOLL";

  state = &_mpe_states[MPE_PVFS_WRITECONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_WRITECONTIG";

  state = &_mpe_states[MPE_PVFS_WRITESTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "PVFS_WRITESTRIDED";

  state = &_mpe_states[MPE_NFS_CLOSE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_CLOSE";

  state = &_mpe_states[MPE_NFS_READDONE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_READDONE";

  state = &_mpe_states[MPE_NFS_WRITEDONE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_WRITEDONE";

  state = &_mpe_states[MPE_NFS_FCNTL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_FCNTL";

  state = &_mpe_states[MPE_NFS_FLUSH_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_FLUSH";

  state = &_mpe_states[MPE_NFS_SETINFO_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_SETINFO";

  state = &_mpe_states[MPE_NFS_IREADCONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_IREADCONTIG";

  state = &_mpe_states[MPE_NFS_IREADSTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_IREADSTRIDED";

  state = &_mpe_states[MPE_NFS_IWRITECONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_IWRITECONTIG";

  state = &_mpe_states[MPE_NFS_IWRITESTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_IWRITESTRIDED";

  state = &_mpe_states[MPE_NFS_OPEN_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_OPEN";

  state = &_mpe_states[MPE_NFS_READSTRIDEDCOLL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_READSTRIDEDCOLL";

  state = &_mpe_states[MPE_NFS_READCONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_READCONTIG";

  state = &_mpe_states[MPE_NFS_READSTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_READSTRIDED";

  state = &_mpe_states[MPE_NFS_RESIZE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_RESIZE";

  state = &_mpe_states[MPE_NFS_SEEKINDIVIDUAL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_SEEKINDIVIDUAL";

  state = &_mpe_states[MPE_NFS_READCOMPLETE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_READCOMPLETE";

  state = &_mpe_states[MPE_NFS_WRITECOMPLETE_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_WRITECOMPLETE";

  state = &_mpe_states[MPE_NFS_WRITESTRIDEDCOLL_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_WRITESTRIDEDCOLL";

  state = &_mpe_states[MPE_NFS_WRITECONTIG_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_WRITECONTIG";

  state = &_mpe_states[MPE_NFS_WRITESTRIDED_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_WRITESTRIDED";

  state = &_mpe_states[MPE_NFS_GET_SHARED_FP_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_GET_SHARED_FP";

  state = &_mpe_states[MPE_NFS_SET_SHARED_FP_ID];
  state->kind_mask = MPE_KIND_ADIO;
  state->name = "NFS_SET_SHARED_FP";
  ...
  /* Enable the basic _mpe_states */
  for (i=0; i<MPE_MAX_STATES; i++) {
  	if ((_mpe_states[i].kind_mask & allow_mask) != 0) 
	    _mpe_states[i].is_active = 1;
  }
