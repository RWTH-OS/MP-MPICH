/* 
 *   $Id: adioi.h 922 2001-05-31 14:10:48Z karsten $    
 *
...
#define ADIOI_Error(fd,error_code,string) ADIOI_ERROR(fd,error_code,string,__FILE__,__LINE__);
...
#include <winbase.h>
#define ADIOI_WRITE_LOCK(fd, offset, whence, len) \
          ADIOI_Set_lock((fd)->fd_sys, LOCKFILE_EXCLUSIVE_LOCK, offset, whence, len)
#define ADIOI_READ_LOCK(fd, offset, whence, len) \
          ADIOI_Set_lock((fd)->fd_sys, 0, offset, whence, len)
#define ADIOI_UNLOCK(fd, offset, whence, len) \
          ADIOI_Remove_lock((fd)->fd_sys, offset, whence, len)
