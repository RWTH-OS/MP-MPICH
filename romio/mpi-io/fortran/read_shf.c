/* 
 *   $Id: read_shf.c 3744 2005-07-18 10:55:08Z georg $    
 *
...
#if defined(FORTRANCAPS)
void FORTRAN_API MPI_FILE_READ_SHARED (MPI_Fint *fh,void *buf,int *count,
                  MPI_Fint *datatype,MPI_Status *status, int *ierr ) 
		  __attribute__ ((weak, alias ("PMPI_FILE_READ_SHARED")));
#elif defined(FORTRANDOUBLEUNDERSCORE)
void FORTRAN_API mpi_file_read_shared__ (MPI_Fint *fh,void *buf,int *count,
                  MPI_Fint *datatype,MPI_Status *status, int *ierr ) 
		  __attribute__ ((weak, alias ("pmpi_file_read_shared__")));
#elif !defined(FORTRANUNDERSCORE)
void FORTRAN_API mpi_file_read_shared (MPI_Fint *fh,void *buf,int *count,
                  MPI_Fint *datatype,MPI_Status *status, int *ierr ) 
		  __attribute__ ((weak, alias ("pmpi_file_read_shared")));
#else
void FORTRAN_API mpi_file_read_shared_ (MPI_Fint *fh,void *buf,int *count,
                  MPI_Fint *datatype,MPI_Status *status, int *ierr ) 
		  __attribute__ ((weak, alias ("pmpi_file_read_shared_")));
#endif
