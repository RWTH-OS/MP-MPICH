/* 
 *   $Id: write_atallf.c 3744 2005-07-18 10:55:08Z georg $    
 *
...
#if defined(FORTRANCAPS)
void FORTRAN_API MPI_FILE_WRITE_AT_ALL (MPI_Fint *fh,MPI_Offset *offset,void *buf,
                          int *count,MPI_Fint *datatype,
                          MPI_Status *status, int *ierr ) 
			  __attribute__ ((weak, alias ("PMPI_FILE_WRITE_AT_ALL")));
#elif defined(FORTRANDOUBLEUNDERSCORE)
void FORTRAN_API mpi_file_write_at_all__ (MPI_Fint *fh,MPI_Offset *offset,void *buf,
                          int *count,MPI_Fint *datatype,
                          MPI_Status *status, int *ierr ) 
			  __attribute__ ((weak, alias ("pmpi_file_write_at_all__")));
#elif !defined(FORTRANUNDERSCORE)
void FORTRAN_API mpi_file_write_at_all (MPI_Fint *fh,MPI_Offset *offset,void *buf,
                          int *count,MPI_Fint *datatype,
                          MPI_Status *status, int *ierr ) 
...
void FORTRAN_API mpi_file_write_at_all_ (MPI_Fint *fh,MPI_Offset *offset,void *buf,
                          int *count,MPI_Fint *datatype,
                          MPI_Status *status, int *ierr ) 
			  __attribute__ ((weak, alias ("pmpi_file_write_at_all_")));
#endif
