...
dnl
define(wi_EXTRA_IDIR, [
incdir="$1"
if test -r $incdir ; then
	case "$CXXFLAGS" in
		*-I${incdir}*)
			# echo "   + already had $incdir" 1>&1
			;;
		*)
...
dnl
dnl
define(wi_EXTRA_LDIR, [
libdir="$1"
if test -r $libdir ; then
	case "$LDFLAGS" in
		*-L${libdir}*)
			# echo "   + already had $libdir" 1>&1
			;;
		*)
...
dnl
dnl
define(wi_EXTRA_PDIR, [
progdir="$1"
if test -r $progdir ; then
	case "$PATH" in
		*:${progdir}*)
			# echo "   + already had $progdir" 1>&1
			;;
		*${progdir}:*)
			# echo "   + already had $progdir" 1>&1
			;;
...
dnl
define(wi_EXTRA_DIRS, [echo "checking for extra include and lib directories..." 1>&1
ifelse([$1], yes, [dnl
b1=`cd .. ; pwd`
b2=`cd ../.. ; pwd`
