...
{
    MPI_Init( &argc, &argv );
	if (verbose) printf( "About to test communicators\n" );fflush(stdout);
    test_communicators();
	if (verbose) printf( "About to Test_Waitforall\n" );fflush(stdout);
    Test_Waitforall( );
    MPI_Finalize();
    return 0;
}
 ...
 */
if (verbose) printf( "Check Comm_dup by adding attributes to lo_comm & duplicating\n" );fflush(stdout);
if (world_rank == 0) 
    printf( "    Comm_dup\n" );
 ...
 */
if (verbose) printf( "Split the world into even & odd communicators with reversed ranks.\n" );fflush(stdout);
      if (world_rank == 0) 
	  printf( "    Comm_split\n" );
 ...
 */
	  if (verbose) printf( "Test each possible Comm_compare result.\n" );fflush(stdout);
      if (world_rank == 0) 
         printf( "    Comm_compare\n" );
 ...
 */
	  if (verbose) printf( "Free all communicators created.\n" );fflush(stdout);
    if (world_rank == 0) 
	printf( "    Comm_free\n" );
