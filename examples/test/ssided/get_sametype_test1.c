	...
	char	** argv;
{
	...
	mysize = sizeof (struct test_t) * 3 * numprocs;
	...
	if (myrank == 0) {
		printf ("get_sametype test 1 (local private; n=%d) ...", numruns);
		if (dodebug) printf ("\n");
		fflush (stdout);
	...
	prnt (myrank, "get_same_type test no. 1\n");
	...
	for (j=0; j<numruns; j++) {
		...
		if (myrank == 0) {
			for (i=0; i<numprocs; i++) {
				...
				MPI_Get (test_val, 3, dtype, i, 0, 3, dtype, win);
				for (k=0; k<3; k++) {
