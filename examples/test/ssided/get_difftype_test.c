	...
	char	** argv;
{
	...
	mysize = (sizeof (struct send_t) + sizeof (struct recv_t)) * numprocs;
	...
	if (myrank == 0) {	
		printf ("get different type (n=%d) ...", numruns);
		if (dodebug) printf ("\n");
		fflush (stdout);
	...
	MPI_Win_create (base, mysize, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &win);

	prnt (myrank, "get_diff_type test\n");
	...
	for (j=0; j<numruns; j++) {
		...
		if (myrank == 0) {
			for (i=0; i<numprocs; i++) {
				recv_val = (struct recv_t *) buffer;
				prnt (myrank, "MPI_Get\n");
				MPI_Get (recv_val, 3, dtype_s, i, 0, 3, dtype_r, win);
				for (k=0; k<3; k++) {
