...
{
	int rank,mysize; /* My Rank (0 or 1) */
	int act_size = 1000;
	int flag,i;
	int buffer[SIZE];
	double t0;
	...
	if (rank == src) { 
		...
		printf("process %i not calculating\n",rank);
		fflush(stdout);
		Test_Waitforall();
		MPI_Finalize();
	}
