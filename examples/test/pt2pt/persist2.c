...
#ifdef DEBUG
	if (rank == 0) printf("buffer attached\n");fflush(stdout);
#endif
...
#ifdef DEBUG
	if (rank == 0) printf("MPI_Recv_init passed\n");fflush(stdout);
#endif
...
#ifdef DEBUG
	if (rank == 0) printf("MPI_Bsend_init passed\n");fflush(stdout);
#endif
    MPI_Startall( 4, r );
#ifdef DEBUG
	if (rank == 0) printf("first startall passed\n");fflush(stdout);
#endif
    MPI_Waitall( 4, r, statuses );
#ifdef DEBUG
	if (rank == 0) printf("first waitall passed\n");fflush(stdout);
#endif
    for (i=0; i<4; i++) {
