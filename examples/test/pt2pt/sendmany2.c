...
int main ( int argc, char *argv[] )
{
  ...
  for (size = 1; size  <= MYBUFSIZE ; size += size) {
	  ...
	  for (i = 0; i < npes; i++) {
	      if (i == self)
		continue;
	      MPI_Irecv (buffer[i], size, MPI_INT, i, MPI_ANY_TAG, MPI_COMM_WORLD, &request[i]);
	    }

	  for (i = 0; i < npes; i++) {
	      if (i == 0)
		continue;
	      MPI_Send (buffer[self], size, MPI_INT, (self +i)%npes, 0, MPI_COMM_WORLD);
	    }

	  for (i = 0; i < npes; i++) {
	      if (i == 0)
		continue;
	      MPI_Wait (&request[(self +i)%npes], &status);
	  }
