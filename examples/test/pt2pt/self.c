...
/*	if (size > 1)
	{
		printf("this test is for only one process");
	}
	*/
	...
	if (rank < 1) fprintf( stdout, "[%d] MPI initialized\n", rank );fflush(stdout);
    ...
    if (rank < 1) fprintf( stdout, "[%d] Starting waitall\n", rank );fflush(stdout);
    mpi_errno = MPI_Waitall ( 2, req, status_array );
    if (rank < 1) fprintf( stdout, "[%d] Ending waitall\n", rank );fflush(stdout);
    ...
    for (i=16; i<257000; i *= 2) {
	  ...
	  if (rank < 1) fprintf( stdout, "[%d] Starting waitall (%d)\n", rank, i );
	  mpi_errno = MPI_Waitall ( 2, req, status_array );
	  if (rank < 1) fprintf( stdout, "[%d] Ending waitall\n", rank );
