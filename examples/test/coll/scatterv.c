 ...
 * correctly.  It requires number of processors that can be split with
 * MPI_Dims_create.
 *
 */
...
void SetData(double *sendbuf, double *recvbuf, int nx, int ny, int myrow,
		int mycol, int nrow, int ncol)
{
	int coldim, i, j, m, k;
	double *p;
...
int CheckData(double* recvbuf, int nx, int ny, int myrow, int mycol, int nrow)
{
	int coldim, m, k;
	double *p, val;
	int errs = 0;
