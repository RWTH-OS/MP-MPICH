 ...
 * $Id$
 *
 * This program provides some simple verification of the MPI_Barrier program.
 * All of the clients send a message to indicate that they are alive (a simple
 * character string) and then the all of the clients enter an MPI_Barrier.  The
 * server then Iprobes for a while to make sure that none of the "through
 * barrier" messages that the clients send after leaving the barrier arive
 * before the server enters the barrier. The server then enters the barrier,
 * and upon leaving, waits for a message from each client.
 */
...
{
	int rank, size, i, recv_flag, ret = 0, passed;
	MPI_Status Status;
	char message[17];
	MPI_Init(&argc, &argv);
