...
MPI_Comm GetNextComm(void)
{
	...
	case 3:
		/* Another division */
		MPI_Comm_split(MPI_COMM_WORLD, rank < (size / 3), size - rank,
				&comm);
		break;
	case 4:
		/* odd and even */
...
int main(int argc, char **argv)
{
	...
	while ((comm = GetNextComm()) != MPI_COMM_NULL) {
		...
		if (result != correct_result) {
			Test_Failed("suming ints with scan");
		}

		MPI_Scan(&data, &result, 1, MPI_INT, MPI_SUM, comm);
		if (result != correct_result) {
			Test_Failed("summing ints with scan (2)");
		}
		...
		if (result != correct_result)
			Test_Failed("summing ints with scan (userop)");

		MPI_Scan(&data, &result, 1, MPI_INT, op_addem, comm);
		if (result != correct_result)
			Test_Failed("summing ints with scan (userop2)");

		result = -100;
		data = rank;
		MPI_Scan(&data, &result, 1, MPI_INT, op_assoc, comm);
		if (result == BAD_ANSWER)
			Test_Failed("scanning with non-commutative op");
		MPI_Comm_free(&comm);
	}
