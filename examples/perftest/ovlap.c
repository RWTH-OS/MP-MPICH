...
OverlapData *ctx;
{
  ...
  myproc = __MYPROCID;
#ifdef USE_SMIMEM
  SMI_Init_reginfo (&shreg_info, len, myproc);
  if (SMI_Create_shreg(SMI_SHM_LOCAL, &shreg_info, &region_id[0], (char **)&sbuffer) != SMI_SUCCESS) {
    printf ("Could not allocate SMI memory.\n");
    MPI_Abort(MPI_COMM_WORLD, -1);
  }
  if (SMI_Create_shreg(SMI_SHM_LOCAL, &shreg_info, &region_id[1], (char **)&rbuffer) != SMI_SUCCESS) {
    printf ("Could not allocate SMI memory.\n");
    MPI_Abort(MPI_COMM_WORLD, -1);
  }
#else
...
#ifdef USE_SMIMEM
  SMI_Free_shreg (region_id[0]); SMI_Free_shreg (region_id[1]); 
#else
  free(sbuffer);
  free(rbuffer);
#endif
