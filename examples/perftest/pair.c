...
{
  double elapsed_time;
  int  i,myproc, to = ctx->destination, from = ctx->source, nbr_errs;
  char *sbuffer,*rbuffer;
  double t0, t1;
  MPI_Status status;

  sbuffer = (char *)mpp_alloc(len);
  rbuffer = (char *)mpp_alloc(len);
  init_rbuffer(rbuffer, len);
  init_sbuffer(sbuffer, len);
...
#if ENABLE_VERIFY
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Blocking exchange (head-to-head), size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
...
PairData *ctx;
{
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Nonblocking exchange (head-to-head), size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
PairData *ctx;
{
  ...
  int            i,myproc, d1, *dmy = &d1, 
                 to = ctx->destination, from = ctx->source, nbr_errs;
  MPI_Request  msg_id;
  MPI_Status   status;
  char           *sbuffer,*rbuffer;
  double   t0, t1;
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Ready send (head-to-head), size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
PairData *ctx;
{
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Blocking round trip, size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
PairData *ctx;
{
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Ready receiver round trip, size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
PairData *ctx;
{
  ...
  if(ctx->is_master){
    ...
    t0=MPI_Wtime();
    for(i=0;i<reps;i++){
      MPI_Isend(sbuffer,len,MPI_BYTE,to,MSG_TAG(i),MPI_COMM_WORLD,&(rid));
      MPI_Recv(rbuffer,len,MPI_BYTE,MPI_ANY_SOURCE,MSG_TAG(i),
		MPI_COMM_WORLD,&status);
      MPI_Wait(&(rid),&status);
#if ENABLE_VERIFY
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Nonblocking round trip, size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
  ...
  if(ctx->is_slave){
    MPI_Isend(sbuffer,len,MPI_BYTE,from,0,MPI_COMM_WORLD,&(rid));
    MPI_Recv(rbuffer,len,MPI_BYTE,MPI_ANY_SOURCE,MSG_TAG(i),
	      MPI_COMM_WORLD, &status);
#if ENABLE_VERIFY
    nbr_errs = check_rbuffer(rbuffer, len);
...
PairData *ctx;
{
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Persistent round trip, size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
double round_trip_vector(int reps, int len, PairData *ctx)
{
  ...
  myproc = __MYPROCID;
  sbuffer = (double *)mpp_alloc((unsigned)(VectorStride * len * sizeof(double) ));
  rbuffer = (double *)mpp_alloc((unsigned)(VectorStride * len * sizeof(double) ));
  init_rbuffer(rbuffer, VectorStride * len * sizeof(double));
  init_sbuffer(sbuffer, VectorStride * len * sizeof(double));
  ...
  if(ctx->is_master){
    ...
    nbr_errs = check_rbuffer(rbuffer, len);
    if (nbr_errs > 0) 
	fprintf (stderr, "[%d] Vector blocking round trip, size %d: %d errors in buffer from %d\n",
		 myproc, len, nbr_errs, from);
#endif
    t0=MPI_Wtime();
    for(i=0;i<reps;i++){
...
PairData *ctx;
{
  ...
  sbuffer = (double *)mpp_alloc((unsigned)(VectorStride * len * sizeof(double) ));
  rbuffer = (double *)mpp_alloc((unsigned)(VectorStride * len * sizeof(double) ));
  init_rbuffer(rbuffer, (unsigned)(VectorStride * len * sizeof(double) ));
  init_sbuffer(sbuffer, (unsigned)(VectorStride * len * sizeof(double) ));
  ...
  if(ctx->is_master){
    ...
    for(i=0;i<reps;i++){
      ...
      nbr_errs = check_rbuffer(rbuffer, len);
      if (nbr_errs > 0) 
	  fprintf (stderr, "[%d] Vector-type blocking round trip, size %d: %d errors in buffer from %d\n",
		   myproc, len, nbr_errs, from);
#endif
    }
    t1=MPI_Wtime();
...
PairData *ctx;
{
  ...
  myproc = __MYPROCID;
  sbuffer = (char *)mpp_alloc((unsigned)(2 * CacheSize ));
  slast   = sbuffer + 2 * CacheSize - len;
  rbuffer = (char *)mpp_alloc((unsigned)(2 * CacheSize ));
