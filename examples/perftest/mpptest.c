     ...
     TwinTest    *ctx;
{
    ...
    if (rank == 0 && max_run_time > 0 && MPI_Wtime() - start_time > max_run_time) {
...
static void SetSizeList(int *sizevals, int *nsizes, int **sizelist)
{
    int i, nbr_msgs, current_size, next_regsize, size_inc;

    /* calculate worst-case size of list */
    for (i = 1, nbr_msgs = 1; i < sizevals[1]; i <<= 1)
	nbr_msgs++;
    nbr_msgs *= (sizevals[2] + 1);
    *sizelist = (int *)malloc (nbr_msgs*sizeof(int));

    *nsizes = 1;
    nbr_msgs = 0;
    (*sizelist)[0] = current_size = sizevals[0];
    while ((current_size << 1) <= sizevals[1]) {
	next_regsize = current_size << 1;
	size_inc = (next_regsize - current_size)/(sizevals[2] + 1);
	if (size_inc == 0)
	    size_inc = next_regsize - current_size;
	
	i = 0;
	do {
	    current_size += size_inc;
	    (*sizelist)[++nbr_msgs] = current_size;
	    i++;
	    (*nsizes)++;
	} while (current_size + size_inc < next_regsize && i <= sizevals[2]);
	
	if (current_size < next_regsize) {
	    (*sizelist)[++nbr_msgs] = next_regsize;
	    current_size = next_regsize;
	    (*nsizes)++;
	}
    }
...
int main( int argc, char *argv[])
{
    ...
    void *outctx;
    void (*ChangeDist)() = 0;
    int  reps,proc1,proc2,error_flag,distance_flag,distance;
    int  first,last,incr, svals[3], expsvals[3];
    ...
    SYArgGetIntVec( &argc, argv, 1, "-expsize", 3, expsvals );
    if (expsvals[1] > 0)
	SetSizeList(expsvals, &nsizes, &sizelist);
    else {
	sizelist = (int *)malloc(DEFAULT_NBR_SIZES*sizeof(int));
	nsizes = SYArgGetIntList( &argc, argv, 1, "-sizelist", DEFAULT_NBR_SIZES, sizelist );
    }
    ...
    do_verify = SYArgHasName( &argc, argv, 1, "-verify");
#if !ENABLE_VERIFY
    if (do_verify) {
	fprintf (stderr, 
"Verification not enabled.\n\
    Set ENABLE_VERIFY to 1 in mpptest.h and recompile.\n");
	MPI_Abort (MP_COMM_WORLD, -1);
    }
#endif
    
    use_mpi_alloc = SYArgHasName( &argc, argv, 1, "-mpialloc");
#if !HAVE_MPI_ALLOC_MEM
    if (use_mpi_alloc) {
	fprintf (stderr, 
"MPI_Alloc_mem() not available.\n\
    Set HAVE_MPI_ALLOC_MEM to 1 in mpptest.h and recompile.\n");
	MPI_Abort (MP_COMM_WORLD, -1);
    }
#endif
...
\n\
  ...
  -verify      Verify transmitted messages for correctness\n\
               (only for point-2-point, and not in conjunction with -cachesize)\n\
  -mpialloc    Allocate buffer memory via MPI_Alloc_mem()\n\
\n\
  ...
  -expsize start end intermediate \n\
               Messages of length (start = start * 2) until the length is greater than end.\n\
               Additionally, 'intermediate' nbr of linearily interpolated messages are \n\
               measured between each of these exponential message sizes. \n\
  -sizelist n1,n2,...\n\
...
{
    /* Check for max time exceeded */
    if (__MYPROCID == 0 && max_run_time > 0 && MPI_Wtime() - start_time > max_run_time) {
	fprintf( stderr, "Exceeded %f seconds, aborting\n", max_run_time );
	MPI_Abort( MPI_COMM_WORLD, 1 );
    }
}
