...
EXPORT_MPI_API int MPI_Type_struct(int count, int blocklens[], MPI_Aint indices[], 
				MPI_Datatype old_types[],
				MPI_Datatype *newtype ) __attribute__ ((weak, alias ("PMPI_Type_struct")));
  ...
  *newtype = (MPI_Datatype) MPIR_FromPointer( dteptr );
  ...
  for (i = 0; i < count; i++)  {
	...
	/* Runtime ERROR #3 occurs in the following line:
		ub_marker is used without being defined
	*/
