...
EXPORT_MPI_API int MPI_Bsend(void *buf, int count, MPI_Datatype datatype,
							int dest, int tag, MPI_Comm comm ) __attribute__ ((weak, alias ("PMPI_Bsend")));
	...
	MPI_Comm comm )
{
	...
	/* 
	   ? BsendDatatype?
	   MPID_BsendContig( buf, len, src_lrank, tag, context_id,
