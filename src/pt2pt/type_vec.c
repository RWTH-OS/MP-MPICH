/*
 *  $Id: type_vec.c,v 1.3 2003/06/05 11:50:37 rainer Exp $
 *
...
EXPORT_MPI_API int MPI_Type_vector(int count, int blocklen, int stride, 
	...
	MPI_Datatype *newtype )
{
  ...
  if ( (blocklen == stride) || (count == 1) ) {
      MPIR_CALL_POP(MPI_Type_contiguous (count * blocklen, old_type, newtype ),MPIR_COMM_WORLD,myname);
  } else {
      /* Reduce this to the hvector case */
      mpi_errno = MPI_Type_hvector ( count, blocklen, (MPI_Aint)stride * old_dtype_ptr->extent,
				     old_type, newtype );
  }
