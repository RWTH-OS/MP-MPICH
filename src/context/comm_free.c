...
EXPORT_MPI_API int MPI_Comm_free ( MPI_Comm *commp ) __attribute__ ((weak, alias ("PMPI_Comm_free")));
...
EXPORT_MPI_API int MPI_Comm_free ( MPI_Comm *commp )
{
  ...
  if ( comm->ref_count <= 0 ) {
	...
	if (comm->comm_coll != comm) {
	  MPID_THREAD_DS_LOCK_FREE(comm->comm_coll);
	}
