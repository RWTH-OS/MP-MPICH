...
#ifdef _WIN32
  if(attr_key->copy_fn.f77_wrapper) {
      /* This must be a fortran routine using STDCALL convention.
         So call the wrapper instead of the real routine.
      */
	MPI_Aint  invall = (MPI_Aint)node->value;
	int inval = (int)invall;
	attr_key->copy_fn.f77_wrapper(attr_key->copy_fn.f77_copy_fn,
				      comm->self, &node->keyval->self, 
...
#endif
#ifdef _WIN32
  if(attr_key->delete_fn.f77_wrapper) {
      MPI_Aint  invall = (MPI_Aint)node->value;
      int inval = (int)invall;
      /* We may also need to do something about the "comm" argument */
       attr_key->delete_fn.f77_wrapper(attr_key->delete_fn.f77_delete_fn,
				       comm->self, 
				       &node->keyval->self, 
				       &inval, 
...
#endif
   /* To stop compiler warning: This seems to make the most sense. Could somebody check this?!
      if ( ( comm != MPI_COMM_NULL ) && ( comm->attr_cache != (MPIR_HBT)0 ) ) { */
  if ( ( comm != (struct MPIR_COMMUNICATOR *)0 ) && ( comm->attr_cache != (MPIR_HBT)0 ) ) {
...
#ifdef _WIN32
  new_key->copy_fn.f77_wrapper = copy_wrapper;
  new_key->delete_fn.f77_wrapper = delete_wrapper;
#endif
  new_key->ref_count	  = 1;
