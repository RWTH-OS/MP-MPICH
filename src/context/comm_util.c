/*
 *  $Id: comm_util.c,v 1.8 2004/06/08 14:34:36 joachim Exp $
 *
			  ...
			  MPIR_COMM_TYPE comm_type )
{
  ...
  MPID_THREAD_DS_LOCK_INIT(new_comm);
  return(MPI_SUCCESS);
}
...
MPIR_Comm_make_coll - make a hidden communicator for one-sided operations
                      from an inter- or intra-communicator assuming
		      that an appropriate number of contexts
		      have been allocated.  An inter-communicator
		      collective can only be made from another
...
See comm_create.c for code that creates a visible communicator.
*/
int MPIR_Comm_make_onesided ( struct MPIR_COMMUNICATOR *comm, 
			      MPIR_COMM_TYPE comm_type )
{
#if 0
  struct MPIR_COMMUNICATOR *new_comm;
  ...
  new_comm->comm_onesided       = new_comm;  /* a circular reference to myself */
  comm->comm_onesided           = new_comm;
	...
	MPIR_COMM_TYPE comm_type)
{
  ...
  new_comm->adiCollCtx         = 0;
  new_comm->collops	       = 0;
  new_comm->attr_cache	       = 0;
  new_comm->recv_context       = -1;
  new_comm->send_context       = -1;
}
...
void MPIR_Comm_collops_init( 
	...
	MPIR_COMM_TYPE comm_type)
{  
    /* first, set the collops to default values... */
    comm->collops = (comm_type == MPIR_INTRA) ? MPIR_intra_collops : MPIR_inter_collops ;

    /* ...then, let the device(s) replace some if they like */
    MPID_Collops_init(comm, comm_type);
