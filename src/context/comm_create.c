/*
 *  $Id: comm_create.c,v 1.4 2003/06/05 11:50:33 rainer Exp $
 *
...
EXPORT_MPI_API int MPI_Comm_create ( MPI_Comm comm, MPI_Group group,
				MPI_Comm *comm_out ) __attribute__ ((weak, alias ("PMPI_Comm_create")));
...
EXPORT_MPI_API int MPI_Comm_create ( MPI_Comm comm, MPI_Group group, MPI_Comm *comm_out )
{
  ...
  if (group_ptr->local_rank == MPI_UNDEFINED) {
    ...
    *comm_out = new_comm->self;
    ...
    (void) MPIR_Comm_make_onesided( new_comm, MPIR_INTRA );
