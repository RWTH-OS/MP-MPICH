...
EXPORT_MPI_API int MPI_Errhandler_create( MPI_Handler_function *function,
				MPI_Errhandler *errhandler) __attribute__ ((weak, alias ("PMPI_Errhandler_create")));
...
{
    return MPIR_Create_user_errhandler((void*)function,MPIR_COMM_HANDLER,errhandler);
}


int MPIR_Create_user_errhandler( 
	void *function,
	...
	MPI_Errhandler       *errhandler)
{
    ...
    new->routine   = (MPI_Handler_function*) function;
    new->ref_count = 1;
    new->type = type;
