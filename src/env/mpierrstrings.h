...
static char *(kind12[]) = {
   ...
   "",
   "The errhandler argument is of wrong type",
   "The errhandler is not of type %s",
   /* ROMIO errors */
...
static char *(kind29[]) = {
   "Invalid MPI_Info key address",
    "",
   "MPI_Info Key length is longer than MPI_MAX_INFO_KEY",
   "MPI_Info Key length is %d; must be no more than MPI_MAX_INFO_KEY=%d",
   "MPI_Info Key is empty",
   "",
    (char *)0};
/* MPI_ERR_INFO_VALUE */
static char *(kind30[]) = {
    "Invalid MPI_Info value address",
    "",
   "MPI_Info Value is longer than MPI_MAX_INFO_VAL",
   "MPI_Info Value length is %d; must be no more than MPI_MAX_INFO_VAL=%d",
   "MPI_Info Value is empty",
   "",
    (char *)0};
/* MPI_ERR_INFO_NOKEY */
static char *(kind31[]) = {
...
static char *(kind32[]) = {
    ...
    "",
    "Can't determine the file-system type. Check the filename/path you provided and try again. Otherwise, prefix the filename with a string to indicate the type of file sytem (piofs:, pfs:, nfs:, ufs:, hfs:, xfs:, sfs:, pvfs:, ntfs:, svm:)",
    "",
    "ROMIO has not been configured to use this file system",
    "ROMIO has not been configured to use the %s file system",
    "",
...
static char *(kind43[]) = {
    "Unsupported datarep",
    "Unsupported datarep %s",
    "Only native data representation currently supported",
    "Datarep %s unsupported. (Only \"native\" currently implemented)",
    (char *)0};
