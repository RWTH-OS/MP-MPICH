	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  static char myname[] = "INTRA_GATHER";
	...
	struct MPIR_COMMUNICATOR *comm )
{
  int        size, rank;
  int        mpi_errno = MPI_SUCCESS;
  static char myname[] = "INTRA_GATHERV";
	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  int        rank, size, i;
  int        mpi_errno = MPI_SUCCESS;
  static char myname[] = "INTRA_SCATTER";
	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  int        rank, size;
  int        mpi_errno = MPI_SUCCESS;
  static char myname[] = "INTRA_SCATTERV";
	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  static char myname[] = "INTRA_ALLTOALL";
	...
	struct MPIR_COMMUNICATOR *comm )
{
    ...
    static char myname[] = "INTRA_REDUCE";
    MPIR_ERROR_DECL;
    mpi_comm_err_ret = 0;
    ...
    */
	...
	recvbuf = (void*)((((size_t)rbuf)+31) & (~31)); /* Create 32 byte aligned buffer*/
#else
    ...
    MPIR_ALLOC(sbuf,(void *)MALLOC(m_extent * count+64),comm, MPI_ERR_EXHAUSTED, 
	"MPI_REDUCE" );
    
    b1 = ((size_t)recvbuf)&31;
    b2 = ((size_t)sbuf)&31;

    buffer= ((char*)sbuf)+(b1+32-b2);
    if (rank != root) recvbuf = (void *)((char*)recvbuf - lb);
    }
#else
	...
	if ((mask & relrank) == 0) {
		...
		if (op_ptr->commute)
#ifdef WIN32
		    if(op_ptr->stdcall) op_ptr->op_s(buffer, recvbuf, &count, &datatype->self); 
		    else
#endif
		    (*uop)(buffer, recvbuf, &count, &datatype->self);
		else {
#ifdef WIN32
		    if(op_ptr->stdcall) op_ptr->op_s(recvbuf, buffer, &count, &datatype->self); 
		    else
#endif
			    ...
			    struct MPIR_COMMUNICATOR *comm )
{
	...
	rc = comm->collops->Bcast (recvbuf, count, datatype, 0, comm);
	if (rc) mpi_errno = rc;
    }
	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  static char myname[] = "INTRA_REDUCE_SCATTER";
  ...
  if (mpi_errno == MPI_SUCCESS || mpi_errno == MPIR_ERR_OP_NOT_DEFINED) {
      rc = comm->collops->Scatterv (buffer, recvcnts, displs, datatype,
				    recvbuf, recvcnts[rank], datatype, 0, comm);
      if (rc) mpi_errno = rc;
  }
	...
	struct MPIR_COMMUNICATOR *comm )
{
  ...
  if (op_ptr->commute) {
	  ...
	  MPIR_ERROR_POP(comm);
#ifdef WIN32
	  if(op_ptr->stdcall) op_ptr->op_s(sendbuf, recvbuf, &count, &datatype->self); 
	  else
#endif
