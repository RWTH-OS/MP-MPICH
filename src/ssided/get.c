/*
 * MPI_Get		reads data from remote memory
 *
 * input parameters:
...
EXPORT_MPI_API int MPI_Get (void *origin_addr, int origin_count, MPI_Datatype origin_datatype, int target_rank, 
			MPI_Aint target_disp, int target_count, MPI_Datatype target_datatype, MPI_Win win) __attribute__ ((weak, alias ("PMPI_Get")));
	...
	MPI_Win 		win;
{
	...
	if (is_contig) {
		...
		error_code = MPID_Get_contig (	origin_addr,
										MIN (target_size, origin_size),
										target_disp, target_rank, dwin,
										NULL, 0);
	} else if (dtypes_equal) {
		error_code = MPID_Get_sametype (origin_addr,
		...
		error_code = MPID_Rhcv (target_rank, dwin, MPID_Hid_Get_emulation,
								vector, 4, NULL);
		FREE (hinfo);
		FREE (vector);
	}
