...
void *MPIR_ToPointer( int idx )
{
    ...
    if (DebugFlag) {
	PRINTF( "ToPointer is %d for pointer %p in block %d\n", 
		idx, PtrBlocks[blocknum][blockidx].ptr, blocknum );
    }
 ...
 */
	if (DebugFlag) {
	    PRINTF( "Pointer %p has idx %d from avail list\n", 
		    ptr, idx );
	}
	return idx;
    }
    ...
    if (DebugFlag) {
	PRINTF( "Pointer %p has idx %d from new block %d at %d\n",
		ptr, idx, blocknum, 1 );
    }
    return idx;
}
	...
	void *ptr)
{
    ...
    if (DebugFlag) {
	PRINTF( "Registered index %d with pointer %p\n", idx, ptr );
    }
}
	...
	FILE *fp )
{
	...
	for (idx = 0; idx < MAX_PTRS; idx++) {
	    if (new[idx].ptr) {
		FPRINTF( fp, "Index %d in use for pointer %p",
			 new[idx].idx, new[idx].ptr );
