...
*/
void MPIR_Pack_Hvector( char *buf, int count, struct MPIR_DATATYPE *datatype, char *outbuf )
{
	...
	int blen   = blocklen * extent;          /* Length of one block in Bytes */
	int c, i, j;
...
void MPIR_UnPack_Hvector( char *inbuf, int count, struct MPIR_DATATYPE *datatype, char *outbuf )
{
	int count1 = datatype->count,            /* Number of blocks */
		blocklen = datatype->blocklen;       /* Number of elements in each block */
	MPI_Aint stride = datatype->stride;      /* Bytes between blocks */
...
int MPIR_HvectorLen( int count, struct MPIR_DATATYPE *datatype)
{
