/* 
 *   $Id: type_get_cont.c,v 1.4 2003/06/05 11:50:35 rainer Exp $    
 *
...
EXPORT_MPI_API int MPI_Type_get_contents(
	...
	MPI_Datatype *array_of_datatypes) __attribute__ ((weak, alias ("PMPI_Type_get_contents")));
	...
	MPI_Datatype *array_of_datatypes)
{
	...
	array_of_integers[2] = dtypeptr->stride/dtypeptr->old_type->extent;
	array_of_datatypes[0] = dtypeptr->old_type->self;
	MPIR_REF_INCR(dtypeptr->old_type);
	break;
    case MPIR_HVECTOR:
	...
	for (; i<=2*dtypeptr->count; i++)
	    array_of_integers[i] = dtypeptr->indices[i-dtypeptr->count-1]/dtypeptr->old_type->extent;
	array_of_datatypes[0] = dtypeptr->old_type->self;
	MPIR_REF_INCR(dtypeptr->old_type);
	break;
    case MPIR_HINDEXED:
