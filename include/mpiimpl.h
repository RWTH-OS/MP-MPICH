...
#else
extern _declspec(dllimport) void *MPIR_F_MPI_BOTTOM;
#endif
#else
extern void *MPIR_F_MPI_BOTTOM;
#endif
...
#  ifdef IN_MPICH_DLL
     ...
     extern _declspec(dllexport) struct MPIR_COMMUNICATOR *MPIR_COMM_LOCAL;
     extern _declspec(dllexport) struct MPIR_COMMUNICATOR *MPIR_COMM_HOST;
     extern _declspec(dllexport) struct MPIR_COMMUNICATOR *MPIR_COMM_META;
     extern _declspec(dllexport) struct MPIR_COMMUNICATOR *MPIR_COMM_ALL;
#  else
     extern _declspec(dllimport) struct MPIR_COMMUNICATOR *MPIR_COMM_WORLD;
     extern _declspec(dllimport) struct MPIR_COMMUNICATOR *MPIR_COMM_LOCAL;
     extern _declspec(dllimport) struct MPIR_COMMUNICATOR *MPIR_COMM_HOST;
     extern _declspec(dllimport) struct MPIR_COMMUNICATOR *MPIR_COMM_META;
     extern _declspec(dllimport) struct MPIR_COMMUNICATOR *MPIR_COMM_ALL;
#  endif /*IN_MPICH_DLL*/
#else
   extern struct MPIR_COMMUNICATOR *MPIR_COMM_WORLD;
   extern struct MPIR_COMMUNICATOR *MPIR_COMM_LOCAL;
   extern struct MPIR_COMMUNICATOR *MPIR_COMM_HOST;
   extern struct MPIR_COMMUNICATOR *MPIR_COMM_META;
   extern struct MPIR_COMMUNICATOR *MPIR_COMM_ALL;
#endif /*WIN32*/
...
/* coll */
#ifdef _M_IX86
extern void MPIR_Setup_Reduce_Ops(void);
#endif
...
int MPIR_Context_dealloc( struct MPIR_COMMUNICATOR *, int, 
				      MPIR_CONTEXT );
int IMPORT_MPI_API MPIR_dup_fn   ( MPI_Comm, int, void *, void *, void *, int * ) ;
...
/* This is used to unify the creation of error handlers.*/
int MPIR_Create_user_errhandler(void *,MPIR_ERRHANDLER_TYPE,MPI_Errhandler*);
