...
#ifdef _WIN32
/* On Win32 platforms we have to take care of Fortran compilers that use
   STDCALL convention. Since only the wrapper knows the convention we need an extra field
   to store a wrapper function in it.
*/
typedef struct {
    void (*f77_wrapper)(void *real_func,MPI_Comm, int *, int *, int *, int *,int *, int * );
    union {
	int (*c_copy_fn)(MPI_Comm, int, void *, void *, void *, int * );
	void (*f77_copy_fn)(MPI_Comm, int *, int *, int *, int *,int *, int * );
    };
} MPIR_Copy_fn;

typedef struct {
    void (*f77_wrapper)(void *real_func,MPI_Comm, int *, int *, void *, int * );
    union {
	int (*c_delete_fn)( MPI_Comm, int, void *, void * );
	void (*f77_delete_fn)( MPI_Comm, int *, int *, void *, int * );
    };
} MPIR_Delete_fn;
#else
typedef union {
