...
DWORD ReadConfigData(HANDLE f,HostData *Host) {
   ...
   size += Read;
   ReadFile(f,&P->LoadProfile,sizeof(P->LockIt),&Read,0);
   size += Read;
...
DWORD ReadAccountEncrypted(HANDLE f,HostData *Host) {
    ...
    if(!ReadFile(f,buffer,DataSize,&Read,0) || !Read)
	    return size;
    size += Read;
    DecryptData(buffer,&DataSize);
    memcpy(Host->Account,buffer+DataSize,sizeof(TAccount));
    return size;
}
...
DWORD WriteAccountEncrypted(HANDLE f,HostData *Host) {
    ...
    //EncryptData((char*)Host->Account,&resbuf,&bufsize); //si check returnvalue
	if (!EncryptData((char*)Host->Account,&resbuf,&bufsize)){
		DWORD errcode;
		errcode = GetLastError();
		DBM("EncryptData failed with errorcode "<< errcode);
		SetLastError(errcode);
	}
	if(!WriteFile(f,&bufsize,sizeof(DWORD),&Written,0)) {
		DWORD errcode;
		errcode = GetLastError();
		DBM("WriteFile failed with errorcode "<< errcode);
		SetLastError(errcode);
