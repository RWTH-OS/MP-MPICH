...
/* 
 *   $Id: ad_fstype.c,v 1.2 2005/03/11 08:45:10 mhs Exp $    
 *
...
static void ADIO_FileSysType_prefix(char *filename, int *fstype, int *error_code)
{
	...
	*fstype = ADIO_TESTFS;
    ...
    else if (!strncmp(filename, "tunnelfs:", 9)
         || !strncmp(filename, "TUNNELFS:", 9))
    {
    *fstype = ADIO_TUNNELFS;
    }
    else if (!strncmp(filename, "memfs:", 6)
         || !strncmp(filename, "MEMFS:", 6))
    {
    *fstype = ADIO_MEMFS;
    }
    else {
...
# endif
	...
	*ops = &ADIO_TESTFS_operations;
    ...
    if (file_system == ADIO_TUNNELFS) {
#ifndef ROMIO_TUNNELFS
# ifdef MPICH2
	*error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, __LINE__, MPI_ERR_IO, "**iofstypeunsupported", 0);
	return;
# elif defined(PRINT_ERR_MSG)
	FPRINTF(stderr, "ADIO_ResolveFileType: ROMIO has not been configured to use the TUNNELFS file system\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
# else /* MPICH-1 */
	myerrcode = MPIR_Err_setmsg(MPI_ERR_IO, MPIR_ERR_NO_TESTFS,
				     myname, (char *) 0, (char *) 0);
...
# endif
	return;
#else
	*ops = &ADIO_TUNNELFS_operations;
#endif
    }
    if (file_system == ADIO_MEMFS) {
#ifndef ROMIO_MEMFS
# ifdef MPICH2
	*error_code = MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE, myname, __LINE__, MPI_ERR_IO, "**iofstypeunsupported", 0);
	return;
# elif defined(PRINT_ERR_MSG)
	FPRINTF(stderr, "ADIO_ResolveFileType: ROMIO has not been configured to use the MEMFS file system\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
# else /* MPICH-1 */
	myerrcode = MPIR_Err_setmsg(MPI_ERR_IO, MPIR_ERR_NO_TESTFS,
				     myname, (char *) 0, (char *) 0);
