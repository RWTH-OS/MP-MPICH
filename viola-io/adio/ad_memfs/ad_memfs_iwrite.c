                ...
                ADIO_Offset offset, ADIO_Request *request, int *error_code)  
{
    ...
    static char myname[] = "ADIOI_MEMFS_IWRITECONTIG";
#endif
#endif

#ifdef DEBUG_MEMFS
    fprintf(stderr, "ADIOI_MEMFS_IwriteContig called\n");
#endif
    ...
    *request = ADIOI_Malloc_request();
    ...
    ADIOI_MEMFS_WriteContig(fd, buf, len, MPI_BYTE, file_ptr_type, offset, 
			  &status, error_code);  
    (*request)->queued = 0;
...
/* call the blocking version. It is faster because it does data sieving. */
    ADIOI_MEMFS_WriteStrided(fd, buf, count, datatype, file_ptr_type, 
...
int ADIOI_MEMFS_aio(ADIO_File fd, void *buf, int len, ADIO_Offset offset,
		  int wr, void *handle)
{
    int err=-1, fd_sys;
       ...
       /* the man pages say EPROCLIM, but in reality errno is set to EAGAIN! */
		...
		if (errno == EAGAIN) {
		    ...
		    if (wr) err = aiowrite(fd_sys, buf, len, offset, SEEK_SET, result); 
                    ...
                    FPRINTF(stderr, "Unknown errno %d in ADIOI_MEMFS_aio\n", errno);
