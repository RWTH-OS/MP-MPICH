...
/* 
 *   $Id: write_sh.c,v 1.2 2005/03/30 11:52:32 mhs Exp $    
 *
                          ...
                          MPI_Datatype datatype, MPI_Status *status)
{
	...
	off = fh->disp + fh->etype_size * shared_fp;
        ...
        /* tunnelfs server recognizes ADIO_SHARED parameter, so there is no
         * need to use an explicit offset */
        if (fh->file_system != ADIO_TUNNELFS)
            ADIO_WriteContig(fh, buf, count, datatype, ADIO_EXPLICIT_OFFSET,
                             off, status, &error_code); 
        else
            ADIO_WriteContig(fh, buf, count, datatype, ADIO_SHARED,
                             off, status, &error_code); 
    ...
    else
    {
        if (fh->file_system != ADIO_TUNNELFS)
            ...
            ADIO_WriteStrided(fh, buf, count, datatype, ADIO_SHARED,
                              shared_fp, status, &error_code);
    }
