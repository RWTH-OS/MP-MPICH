...
if (datatype->dte_type == MPIR_PACKED) {
    MPID_SendContig( buf, count, src_lrank, tag, 
		     context_id, dest_grank, error_code );
    }
else {
	...
	*error_code = MPIR_Pack( comm, buf, count, datatype, tmpbuf, 
	...
	tmpbuf = 0;
    MPID_SendContig( tmpbuf, len, src_lrank, tag, 
		    context_id, dest_grank, error_code );
    if (tmpbuf) {
	FREE( tmpbuf );
	}
...
if (len > 0) {
	...
	*error_code = MPIR_ERROR(comm,MPI_ERR_EXHAUSTED,"");
    ...
    tmpbuf = 0;
MPID_RecvContig( tmpbuf, len, src_lrank, tag, 
...
MPID_IsendContig( tmpbuf, len, src_lrank, tag, 
		        context_id, dest_grank, request, error_code );
return;
}
...
MPID_IrecvContig( tmpbuf, len, src_lrank, tag, 
		       context_id, dest_grank, request, error_code );
return;
}
