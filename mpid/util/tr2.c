...
char     *fname;
{
	...
	fprintf( stderr, "[%d] Allocating %d bytes at %p in %s:%d\n", 
...
/* Don't try to handle empty blocks */
	...
	/* Damaged header */
	fprintf( stderr, "[%d] Block at address %p is corrupted; cannot free;\n\
...
/* Check that nend is properly aligned */
    if ((sizeof(long) == 4 && ((size_t)nend & 0x3) != 0) ||
	(sizeof(long) == 8 && ((size_t)nend & 0x7) != 0)) {
	fprintf( stderr,
 "[%d] Block at address %p is corrupted (invalid address or header)\n\
called in %s at line %d\n", world_rank, a + sizeof(TrSPACE), 
		 file, line );
	return;
    }
...
/* Mark the location freed */
    *nend		   = ALREADY_FREED;
    ...
    if (TRlevel & TR_FREE)
	fprintf( stderr, "[%d] Freeing %lu bytes at %p in %s:%d\n", 
		 world_rank, head->size, a + sizeof(TrSPACE),
		 file, line );
...
while (head) {
	...
	errs++;
	fprintf( stderr, "[%d] Block at address %p is corrupted\n", 
                 world_rank, head );
...
FILE *fp;
{
    ...
    head = TRhead;
    while (head) {
	fprintf( fp, "[%d] %lu at [%p], id = ", 
		 world_rank, head->size, head + sizeof(TrSPACE) );
	if (head->id >= 0) {
	    head->fname[TR_FNAME_LEN-1] = 0;
	    fprintf( fp, "%d %s[%d]\n", 
...
/* We should really use the size of the old block... */
    ...
    if (head->cookie != COOKIE_VALUE) {
	/* Damaged header */
	fprintf( stderr, "[%d] Block at address %p is corrupted; cannot realloc;\n\
may be block not allocated with MPID_trmalloc or MALLOC\n", 
		 world_rank, pa );
	return 0;
    }
