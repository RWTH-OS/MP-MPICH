...
void CTCPCommunicator_Destructor(CTCPCommunicator* CTCPCommunicatorPt)
{
  ...
  if(MPID_USOCK_Data_global.Events)
  {
    for(i=0; i<CTCPCommunicatorPt->numProcs; i++) CloseHandle(MPID_USOCK_Data_global.Events[i]);
    free(MPID_USOCK_Data_global.Events);
    MPID_USOCK_Data_global.Events=0;
  }
...
int CTCPCommunicator_Create(int *argc,char*** argv, CTCPCommunicator* CTCPCommunicatorPt)
{
  ...
  MPID_USOCK_Data_global.Events = (HANDLE*)malloc(sizeof(HANDLE)*CTCPCommunicatorPt->numProcs);
  MPID_USOCK_Data_global.Events = CreateEvents(CTCPCommunicatorPt->numProcs,CTCPCommunicatorPt->numProcs);
  
  DSECTLEAVE
    return result;
}

#define HEADERSIZE (sizeof(MessageHeader)+5*sizeof(DWORD))
...
int CTCPCommunicator_MasterConnect(CTCPCommunicator* CTCPCommunicatorPt)
{
  ...
  tempNodes = (NodeDescription*) malloc (CTCPCommunicatorPt->numProcs * sizeof(NodeDescription));
...
int CTCPCommunicator_ClientConnect(unsigned long Master, CTCPCommunicator* CTCPCommunicatorPt)
{
  ...
  if(!CTCPCommunicatorPt->localAddress)
  {
    ...
    else GetLocalIps(msg.body.Data.address,&NumIps,TRUE);
  }
  else 
  {
  ...
  DNOTICE("Send my name to the master");
