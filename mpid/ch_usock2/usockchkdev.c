/*
 *  $Id: usockchkdev.c 3189 2005-03-03 17:12:54Z carsten $
 *
 */
	...
	MPID_BLOCKING_TYPE is_blocking)
{
	...
	if (is_posted) {
		...
		fprintf( stderr, 
			 ...
			 MPID_USOCK_Data_global.MyWorldRank, __FILE__, __LINE__ );
		fflush( stderr );
	    }
	}
	else {
		...
		break;
	    case MPID_PKT_REQUEST_SEND:
		MPID_USOCK_DEBUG_TEST_FCN(dev->rndv->unex,"dev->rndv->unex");
	...
	default:
	    fprintf( stdout, "[%d] Mode %d is unknown (internal error) %s:%d!\n", 
		     MPID_USOCK_Data_global.MyWorldRank, pkt.head.mode, 
		     __FILE__, __LINE__ );
	}
