...
int      partner;
{
    ...
    MPID_IrecvContig( MPI_BOTTOM, 0, partner, 0, 1, request, &err );
    MPID_SendContig( MPI_BOTTOM, 0, MPID_MyWorldRank, 0, 1, partner, 
		     msgrep, &err );
 ...
 */
	...
	merr = 0;
	MPID_RecvContig( recvbuf, sizeof(int), partner, 1, 0, &status, 
			   &merr );
	...
	SetupRecvBuf( recvbuf );
	merr = 0;
	MPID_RecvContig( recvbuf, sizeof(int), partner, 2, 0, &status,
			   &merr );
	err += CheckRecvErr( merr, &status, recvbuf, "Unexpected Recv" );

	SetupRecvBuf( recvbuf );
	merr = 0;
	MPID_IrecvContig( recvbuf, sizeof(int), partner, 3, 0, request, 
			    &merr );
	...
	SetupRecvBuf( recvbuf );
	merr = 0;
	MPID_IrecvContig( recvbuf, sizeof(int), partner, 4, 0, request,
			    &merr );
	fence( comm, partner );
	...
	for (i=0; i<10; i++) 
	    sendbuf[i] = 100 + i;
	MPID_SendContig( sendbuf, 10*sizeof(int), rank, 1, 0, partner, 
			 msgrep, &merr );
	MPID_IsendContig( sendbuf, 10*sizeof(int), rank, 2, 0, 
	...
	MPID_SendContig( sendbuf, 10*sizeof(int), rank, 3, 0, 
			 partner, msgrep, &merr );
	fence( comm, partner );

	MPID_SendContig( sendbuf, 10*sizeof(int), rank, 4, 0, partner, 
			 msgrep, &merr );
    }
