...
char **argv;
{
    ...
    for (j=0; j<nmsgs; j++) {
	MPID_IrecvContig( rbuf, len, master, nmsgs-j-1, 0, 
			  req[nmsgs-j-1], &err );
    }
    for (j=0; j<nmsgs; j++) {
	MPID_SsendContig( sbuf, len, master, j, 0, slave, msgrep, 
			  &err );
    }
    ...
    for (j=0; j<nmsgs; j++) {
	MPID_IssendContig( sbuf, len, master, j+10, 0, slave, 
			   msgrep, req[j], &err );
    }
    /* We must wait on them in RECEIVER order */
    /* Is this a design bug in the ADI? */
    for (j=nmsgs-1; j>=0; j--) {
	MPID_RecvContig( rbuf, len, master, j+10, 0, &status, 
			 &err );
	(void) CheckStatus( &status, master, j, len );
	(void) CheckData( sbuf, rbuf, len );
    }
