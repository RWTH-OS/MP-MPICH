...
int MPID_SHMEM_Rndvn_cancel_recv ANSI_ARGS(( MPIR_RHANDLE *));
 ...
 */
int MPID_SHMEM_Rndvn_isend( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			    msgrep, shandle, dty )
void          *buf;
...
struct MPIR_DATATYPE * dty;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("S About to get pkt for request to send");
    pkt = (MPID_PKT_GET_T *) MPID_SHMEM_GetSendPkt(0);
    /* GetSendPkt hangs untill successful */
    MPID_SHMEM_DEBUG_PRINT_MSG("S Starting Rndvb_isend");
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG(sbuf);
    }
#endif
    ...
    MPID_SHMEM_DEBUG_PRINT_BASIC_SEND_PKT("S Sending rndv-get message",pkt)
#ifndef META
	MPID_n_pending++;
#endif
    MPID_SHMEM_SendControl( (MPID_PKT_T *)pkt, sizeof(MPID_PKT_GET_T), dest_dev_lrank );
 ...
 */
int MPID_SHMEM_Rndvn_send( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			   msgrep, dty )
void          *buf;
int           len, tag, context_id, src_comm_lrank, dest_dev_lrank;
MPID_Msgrep_t msgrep;
struct MPIR_DATATYPE * dty;
{
    ...
    MPID_SHMEM_DEBUG_INIT_STRUCT(&shandle,sizeof(shandle));
    MPIR_SET_COOKIE((&shandle),MPIR_REQUEST_COOKIE);
    MPID_Send_init( &shandle );
    shandle.finish = 0;
    MPID_SHMEM_Rndvn_isend( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			 msgrep, &shandle, dty );
    MPID_SHMEM_DEBUG_TEST_FCN(shandle.wait,"req->wait");
 ...
 */
int MPID_SHMEM_Rndvn_ack( in_pkt, from_dev_lrank )
void  *in_pkt;
int   from_dev_lrank;
...
{  /* begin MPID_SHMEM_Rndvn_ack */
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting Rndvb_ack");
#ifdef MPID_PACK_CONTROL
	...
	/* Continuing data */
	MPID_SHMEM_DEBUG_PRINT_MSG("Sending incremental cont get");
        temp_to     = pkt->to;
	...
	if (pkt->len_avail == 0) {  /* begin if len_avail */
		...
		pkt->address = MPID_SHMEM_SetupGetAddress( shandle->start, 
					    &pkt->len_avail, from_dev_lrank );
	    }  /* end if len_avail > 0 */
	    else 
		pkt->address = 0;
	    pkt->cur_offset = 0;
	...
	MPID_SHMEM_DEBUG_PRINT_BASIC_SEND_PKT("S Sending cont-get message",pkt)
	MPID_SHMEM_SendControl( (MPID_PKT_T *)pkt, 
				sizeof(MPID_PKT_GET_T), from_dev_lrank );
	...
	if (pkt->len_avail + pkt->cur_offset >= rhandle->s.count) {
	    ...
	    MPID_SHMEM_FreeGetAddress( pkt->address );
	    MPID_SHMEM_FreeRecvPkt( (MPID_PKT_T*)pkt );
	}  /* end if len_avail + cur_offset > s.count */

	else {  /* begin else */
	    pkt->mode = MPID_PKT_OK_TO_SEND_GET;
	    MPID_SHMEM_DEBUG_PRINT_BASIC_SEND_PKT("R Sending ok-to-send message",pkt)
#ifdef MPID_PACK_CONTROL
    while (!MPID_PACKET_CHECK_OK(pkt->src)) {  /* begin while !ok loop */
	/* Wait for a protocol ACK packet */
#ifdef MPID_DEBUG_ALL
 ...
 */
int MPID_SHMEM_Rndvn_irecv( rhandle, from_dev_lrank, in_pkt )
...
void         *in_pkt;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting rndvb irecv");
    ...
    rhandle->cancel       = MPID_SHMEM_Rndvn_cancel_recv;

#ifdef MPID_PACK_CONTROL
    while (!MPID_PACKET_CHECK_OK(from_dev_lrank)) {  /* begin while !ok loop */
	/* Wait for a protocol ACK packet */
#ifdef MPID_DEBUG_ALL
	if (MPID_DebugFlag || MPID_DebugFlow)
	    FPRINTF(MPID_DEBUG_FILE,
	 "[%d] S Waiting for a protocol ACK packet (in rndvb isend) from %d\n",
		    MPID_SHMEM_rank, from_dev_lrank);
#endif
	MPID_DeviceCheck( MPID_BLOCKING );
    }  /* end while !ok loop */

    MPID_PACKET_ADD_SENT(pkt->to, from_dev_lrank);
#endif

    /* Send back an "ok to proceed" packet */
    pkt->mode = MPID_PKT_OK_TO_SEND_GET;
    pkt->to         = from_dev_lrank;
...
int MPID_SHMEM_Rndvn_save( rhandle, from_dev_lrank, in_pkt )
MPIR_RHANDLE *rhandle;
int          from_dev_lrank;
void         *in_pkt;
{
    ...
    MPID_PACKET_ADD_RCVD(pkt->to, pkt->src);
#endif
    MPID_SHMEM_DEBUG_PRINT_MSG("Saving info on unexpected message");
    rhandle->s.MPI_TAG	  = pkt->tag;
    rhandle->s.MPI_SOURCE = pkt->lrank;
 ...
 */
int MPID_SHMEM_Rndvn_ok_to_send( send_id, rtag, from_dev_lrank )
MPID_Aint   send_id;
MPID_RNDV_T rtag;
int         from_dev_lrank;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("Starting rndvn ok to send");
    pkt = (MPID_PKT_GET_T *)MPID_SHMEM_GetSendPkt(0);
    /* GetSendPkt hangs untill successful */
    pkt->mode = MPID_PKT_CONT_GET;
    MPID_AINT_SET(pkt->send_id,send_id);
    /* pkt->recv_handle = rtag; */
    MPID_SHMEM_DEBUG_PRINT_BASIC_SEND_PKT("S Ok send", pkt);
    MPID_SHMEM_SendControl( pkt, sizeof(MPID_PKT_GET_T), from_dev_lrank );
    return MPI_SUCCESS;
}
#endif
...
void         *in_runex;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R about to get packet for ok to send");
    pkt = (MPID_PKT_GET_T *) MPID_SHMEM_GetSendPkt(0);
    /* ? test for length ? */
#ifdef MPID_PACK_CONTROL
...
MPIR_SHANDLE *shandle;
{
    MPID_SHMEM_DEBUG_PRINT_MSG("Ending send transfer");
#ifdef FOO
...
MPIR_SHANDLE *shandle;
{
    MPID_SHMEM_DEBUG_PRINT_MSG("Testing for end send transfer" );
#ifdef FOO
...
MPIR_SHANDLE *shandle;
{
    MPID_SHMEM_DEBUG_PRINT_MSG("Waiting for Rndvn ack");
    while (!shandle->is_complete && 
	   shandle->wait == MPID_SHMEM_Rndvn_send_wait_ack) {
	MPID_DeviceCheck( MPID_BLOCKING );
    }
    if (!shandle->is_complete) {
	MPID_SHMEM_DEBUG_TEST_FCN(shandle->wait,"shandle->wait");
...
MPIR_SHANDLE *shandle;
{
    MPID_SHMEM_DEBUG_PRINT_MSG("Testing for Rndvn ack" );
    if (!shandle->is_complete &&
	shandle->test == MPID_SHMEM_Rndvn_send_test_ack) {
	MPID_DeviceCheck( MPID_NOTBLOCKING );
    }
...
int MPID_SHMEM_Rndvn_cancel_recv( rhandle )
MPIR_RHANDLE    *rhandle;
{
#if 0
    /* free recv buffer */
...
MPID_Protocol *MPID_SHMEM_Rndvn_setup()
{
    ...
    p->wait_recv   = 0;
    p->push_recv   = 0;
    p->cancel_recv = MPID_SHMEM_Rndvn_cancel_recv;
