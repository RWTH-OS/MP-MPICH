...
int MPID_SHMEM_Eagern_isend( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			 msgrep, shandle, dty)
void          *buf;
...
struct MPIR_DATATYPE * dty;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("S Starting Eagern_isend");
#ifdef MPID_FLOW_CONTROL
    while (!MPID_FLOW_MEM_OK(len,dest_dev_lrank)) {  /* begin while !ok loop */
    ...
    MPID_FLOW_MEM_SEND(len,dest_dev_lrank); 
#endif
    ...
    pkt->len	    = len;
#ifdef MPID_FLOW_CONTROL
    MPID_FLOW_MEM_ADD(&pkt,dest_dev_lrank);
#endif
    ...
    MPID_SHMEM_DEBUG_PRINT_SEND_PKT("S Sending extra-long message",pkt);

    /* Place in shared memory */
    /* Better if setup fails if full memory not available */
    in_len = len;
    pkt->address = MPID_SHMEM_SetupGetAddress( buf, &len, dest_dev_lrank );
	...
	return MPID_SHMEM_Rndvn_isend( buf, in_len, src_comm_lrank, tag, context_id,
				       dest_dev_lrank, msgrep, shandle );
    }
    ...
    MPID_SHMEM_SendControl( (MPID_PKT_T *)pkt, 
			    sizeof(MPID_PKT_SEND_ADDRESS_T), dest_dev_lrank );
...
int MPID_SHMEM_Eagern_send( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			    msgrep, dty )
void          *buf;
int           len, tag, context_id, src_comm_lrank, dest_dev_lrank;
MPID_Msgrep_t msgrep;
struct MPIR_DATATYPE * dty;
{
    ...
    MPID_SHMEM_Eagern_isend( buf, len, src_comm_lrank, tag, context_id, dest_dev_lrank,
			     msgrep, &shandle, dty );
 ...
 */
int MPID_SHMEM_Eagern_recv( rhandle, from_dev_lrank, in_pkt )
...
void         *in_pkt;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting Eagern_recv");
#ifdef MPID_FLOW_CONTROL 
     MPID_FLOW_MEM_GET(pkt,from_dev_lrank);
#endif
...
#ifdef MPID_FLOW_CONTROL
    MPID_FLOW_MEM_READ(msglen,from_dev_lrank);
    MPID_FLOW_MEM_RECV(msglen,from_dev_lrank);
#endif
...
int MPID_SHMEM_Eagern_irecv( rhandle, from_dev_lrank, in_pkt )
MPIR_RHANDLE *rhandle;
int          from_dev_lrank;
void         *in_pkt;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting Eagern_irecv");
    /* Check for truncation */
    MPID_CHK_MSGLEN(rhandle,msglen,err)
#ifdef MPID_FLOW_CONTROL
    MPID_FLOW_MEM_GET(pkt,from_dev_lrank);
...
int MPID_SHMEM_Eagern_save( rhandle, from_dev_lrank, in_pkt )
MPIR_RHANDLE *rhandle;
int          from_dev_lrank;
void         *in_pkt;
{
    ...
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting Eagern_save");
#ifdef MPID_PACK_CONTROL
    if (MPID_PACKET_RCVD_GET(pkt->src)) {
	MPID_SendProtoAck(pkt->to, pkt->src);
    }
...
#ifdef MPID_FLOW_CONTROL
	MPID_FLOW_MEM_READ(pkt->len,from_dev_lrank);
#endif
...
void         *in_runex;
{
    ...
    MPID_CHK_MSGLEN(rhandle,msglen,err);
    MPID_SHMEM_DEBUG_PRINT_MSG("R Starting unxrecv_start");
#ifdef MPID_PACK_CONTROL
...
#ifdef LEAVE_IN_SHARED_MEM
	MPID_SHMEM_FreeGetAddress( runex->start );
#else
	FREE( runex->start );
#endif
#ifdef MPID_FLOW_CONTROL
