/* $Id: sort_memcpy.c,v 1.1 2004/03/19 22:14:45 joachim Exp $
...
void merge(int phase)
{
    ...
    if (iType == 0) 
      SMI_Memcpy(sgmt2 + (my_rank + 1) * sgmt_size, sgmt + my_rank * sgmt_size,
		 sgmt_size * sizeof(int), 0);
    else
      SMI_Memcpy(sgmt2 + (my_rank - 1) * sgmt_size, sgmt + my_rank * sgmt_size,
		 sgmt_size * sizeof(int), 0);
    ...
    SMI_Memcpy(sgmt + my_rank *sgmt_size, swap, sgmt_size * sizeof(int), 0);
