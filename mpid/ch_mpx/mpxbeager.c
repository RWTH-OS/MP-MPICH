/*
 *  $Id: mpxbeager.c 4397 2006-01-30 10:41:47Z carsten $
 *
...
int MPID_MPX_Eagerb_recv ANSI_ARGS(( MPIR_RHANDLE *, int, void * ));
int MPID_MPX_Eagerb_irecv ANSI_ARGS(( MPIR_RHANDLE *, int, void * ));
int MPID_MPX_Eagerb_save ANSI_ARGS(( MPIR_RHANDLE *, int, void * ));
int MPID_MPX_Eagerb_unxrecv_start ANSI_ARGS(( MPIR_RHANDLE *, void * ));
int MPID_MPX_Eagern_WaitReceive ANSI_ARGS((MPIR_RHANDLE*));
int MPID_MPX_Eagern_TestReceive ANSI_ARGS((MPIR_RHANDLE*));

int MPID_MPX_Eagerb_cancel_recv ANSI_ARGS((MPIR_RHANDLE *)) ;
 ...
 */
int MPID_MPX_Eagerb_recv( rhandle, from, in_pkt )
MPIR_RHANDLE *rhandle;
int          from;
void         *in_pkt;
{
    DSECTION("MPID_MPX_Eagerb_recv");
    ...
    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagerb_recv");
	...
	MPID_MPX_ConsumeData(remain,from);
    }
...
* message
*/
int MPID_MPX_Eagerb_unxrecv_start( rhandle, in_runex )
MPIR_RHANDLE *rhandle;
void         *in_runex;
{
    DSECTION("MPID_MPX_Eagerb_unxrecv_start");
    ...
    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagerb_unxrecv_start");

    msglen = runex->s.count;
    MPID_CHK_MSGLEN(rhandle,msglen,err);
    /* Copy the data from the local area and free that area */
    if(!runex->is_complete)
		MPID_MPX_Eagern_WaitReceive(runex);
...
int MPID_MPX_Eagerb_cancel_recv(runex) 
MPIR_RHANDLE *runex;
{
  /* get the device of this handle: (may be, there a multiple mpx entities) */
  if( runex->dev!=0 )
  ...
  MPID_MPX_Test_device(MPID_devset->active_dev, "cancel_recv");
  
  if(!runex->is_complete) MPID_MPX_WaitTransfer(runex->rid);
  if (runex->s.count > 0) {
    FREE( runex->start );
  }
...
int MPID_MPX_Eagerb_save( rhandle, from, in_pkt )
MPIR_RHANDLE *rhandle;
int          from;
void         *in_pkt;
{
    DSECTION("MPID_MPX_Eagerb_save");
    MPID_PKT_T *pkt = (MPID_PKT_T *)in_pkt;

    DSECTENTRYPOINT;

    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagerb_save");
    ...
    rhandle->cancel	  = MPID_MPX_Eagerb_cancel_recv;
	...
	if (!rhandle->start) {
	    ...
	    MPID_MPX_ConsumeData(pkt->head.len,from);
	    rhandle->push = MPID_MPX_Eagerb_unxrecv_start;
	    DSECTLEAVE
		return 1;
	}
	MPID_FLOW_MEM_READ(pkt->head.len,from);
    }
    if(MPID_MPX_RecvFromChannelAsync( rhandle->start, pkt->head.len, from,rhandle->rid)) {
	if (rhandle->finish)
	    (rhandle->finish)( rhandle );
	rhandle->is_complete  = 1;
...
int MPID_MPX_Eagerb_irecv( rhandle, from, in_pkt )
MPIR_RHANDLE *rhandle;
int          from;
void         *in_pkt;
{
    DSECTION("MPID_MPX_Eagerb_irecv");
    MPID_PKT_LONG_T *pkt = (MPID_PKT_LONG_T *)in_pkt;
    int    msglen, err = MPI_SUCCESS,remain;

    DSECTENTRYPOINT;

    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagerb_irecv");
    ...
    /*if(MPID_MPX_RecvFromChannelAsync( rhandle->buf, msglen, from,rhandle->rid)) {*/
    MPID_MPX_RecvFromChannel( rhandle->buf, msglen, from);
    if (rhandle->finish)
	(rhandle->finish)( rhandle );
    rhandle->is_complete = 1;
    rhandle->wait	 = 0;
    ...
    /*} else {
      ...
      rhandle->wait	 = MPID_MPX_Eagern_WaitReceive;
      rhandle->test	 = MPID_MPX_Eagern_TestReceive;
    }*/
    rhandle->push	 = 0;
...
int MPID_MPX_Eagern_WaitReceive (rhandle) 
MPIR_RHANDLE *rhandle;
{
    DSECTION("MPID_MPX_Eagern_WaitReceive");
    ...
    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagern_WaitReceive");

    MPID_MPX_WaitTransfer(rhandle->rid);
    rhandle->is_complete = 1;
    if (rhandle->finish) 
	(rhandle->finish)( rhandle );
...
int MPID_MPX_Eagern_TestReceive (rhandle) 
MPIR_RHANDLE *rhandle;

{
    DSECTION("MPID_MPX_Eagern_TestReceive");
    ...
    MPID_MPX_Test_device(MPID_devset->active_dev, "Eagern_TestReceive");
    
    if (MPID_MPX_TestTransfer( rhandle->rid )) {
	rhandle->is_complete = 1;
	if (rhandle->finish) 
	    (rhandle->finish)( rhandle );
	MPID_MPX_ReceiveTransfer(-1,rhandle->rid);
    }
