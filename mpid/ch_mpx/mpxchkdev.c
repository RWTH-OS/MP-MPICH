/*
 *  $Id: mpxchkdev.c 4397 2006-01-30 10:41:47Z carsten $
 *
 */
	...
	MPID_BLOCKING_TYPE is_blocking)
{
    ...
    MPID_MPX_DEBUG_PRINT_MSG("Waiting for message to arrive");
    MPID_PKT_WAIT();
    /* 
    ...
    MPID_MPX_DEBUG_PRINT_PKT("R received message",&pkt);

    /* Separate the incoming messages from control messages */
    if (MPID_PKT_IS_MSG(pkt.head.mode)) {
	MPID_MPX_DEBUG_PRINT_RECV_PKT("R rcvd msg",&pkt);
	...
	if (is_posted) {
	    ...
	    case MPID_PKT_SHORT:
		MPID_MPX_DEBUG_TEST_FCN(dev->short_msg->recv,"dev->short->recv");
		...
		*/
		...
		MPID_MPX_DEBUG_TEST_FCN(dev->rndv->irecv,"dev->rndv->irecv");
		...
		*/
	    ...
	    case MPID_PKT_LONG:
		MPID_MPX_DEBUG_TEST_FCN(dev->eager->irecv,"dev->eager->irecv");
		...
		*/
	    ...
	    switch (pkt.head.mode) {
	    case MPID_PKT_SHORT:
		MPID_MPX_DEBUG_TEST_FCN(dev->short_msg->unex,"dev->short->unex");
		...
		*/
		break;
	    case MPID_PKT_REQUEST_SEND:
		MPID_MPX_DEBUG_TEST_FCN(dev->short_msg->unex,"dev->rndv->unex");
		err = MPID_Protocol_call_unex( dev->short_msg->unex,
					       rhandle, from_grank, &pkt,
					       dev );
		/*
		...
		*/
	    ...
	    case MPID_PKT_LONG:
		MPID_MPX_DEBUG_TEST_FCN(dev->eager->unex,"dev->eager->unex");
	...
	case MPID_PKT_OK_TO_SEND:
	    MPID_MPX_DEBUG_TEST_FCN(dev->rndv->do_ack,"dev->rndv->do_ack");
	...
	case MPID_PKT_ANTI_SEND:
	    MPID_MPX_SendCancelOkPacket( &pkt, from_grank ); 
	    break;
	    
	case MPID_PKT_ANTI_SEND_OK:
	    MPID_MPX_RecvCancelOkPacket( &pkt, from_grank ); 
	    break;
    ...
    MPID_MPX_DEBUG_PRINT_MSG("Exiting check_incoming");
    DSECTLEAVE
	return err;
}
