/*
 *  $Id: mpxcancel.c 4397 2006-01-30 10:41:47Z carsten $
 *
...
int MPID_MPX_SendCancelPacket( shandle )
MPIR_SHANDLE *shandle;

{
    DSECTION("MPID_MPX_SendCancelPacket");
...
/*	pkt->to      = shandle->partner;*/
    ...
    MPID_MPX_DEBUG_PRINT_BASIC_SEND_PKT("S Sending anti-send message\n", pkt);
...
void MPID_MPX_SendCancelOkPacket( in_pkt, from )
MPID_PKT_T  *in_pkt;
int  from;

{ 
    DSECTION("MPID_MPX_SendCancelOkPacket");
    ...
    MPID_MPX_Test_device(MPID_devset->active_dev, "SendCancelOkPacket");
	...
	MPID_MPX_DEBUG_PRINT_MSG2("Could not find cancel function, shandle == %x",shandle);
	new_pkt.cancel = 0;
    } else {
	MPID_MPX_DEBUG_PRINT_MSG("Calling rhandle->cancel");
    ...
    MPID_MPX_DEBUG_PRINT_BASIC_SEND_PKT("S Sending anti_send_ok message\n", &new_pkt);
...
void MPID_MPX_RecvCancelOkPacket( in_pkt, from )
MPID_PKT_T  *in_pkt;
int  from;

{ 
    DSECTION("MPID_MPX_RecvCancelOkPacket");
    MPID_PKT_ANTI_SEND_T *pkt = (MPID_PKT_ANTI_SEND_T *)in_pkt;
    MPIR_SHANDLE *shandle=0;
    
    DSECTENTRYPOINT;

    MPID_MPX_Test_device(MPID_devset->active_dev, "RecvCancelOkPacket");
    ...
    MPID_MPX_DEBUG_PRINT_BASIC_SEND_PKT("R Receive anti-send ok message\n", pkt);  
	...
	MPID_MPX_DEBUG_PRINT_MSG("Request has been successfully cancelled");
    }   /* end if pkt->cancel */
    else {
	shandle->is_cancelled = 0;
	MPID_MPX_DEBUG_PRINT_MSG("Unable to cancel request");
    }
