/*
 *  $Id: mpxdebug.c 4397 2006-01-30 10:41:47Z carsten $
 *
 */
/* MP_MPICH global */
...
void MPID_MPX_Get_print_pkt( FILE *, MPID_PKT_T *);
int  MPID_MPX_Rndv_print_pkt(FILE *, MPID_PKT_T *);
void MPID_MPX_Print_Send_Handle( MPIR_SHANDLE * );
int MPID_MPX_Print_mode( FILE*,MPID_PKT_T * );

/* Should each mode have its own print routines? */

int MPID_MPX_Rndv_print_pkt( fp, pkt )
FILE       *fp;
MPID_PKT_T *pkt;
{
    DSECTION("MPID_MPX_Rndv_print_pkt");
...
int MPID_MPX_Print_packet( fp, pkt )
FILE        *fp;
MPID_PKT_T  *pkt;
{
    DSECTION("MPID_MPX_Print_packet");
    DSECTENTRYPOINT;
...
\tmode       = ", 
	...
	MPID_MPX_Rndv_print_pkt( fp, pkt );
	break;
    case MPID_PKT_FLOW:
...
\tflow info  = %d\n", pkt->head.flow_info );
	...
	MPID_MPX_Get_print_pkt( fp, pkt );
	break;
    default:
	fprintf( fp, "\n" );
    }
    MPID_MPX_Print_mode( fp, pkt );
...
void MPID_MPX_Get_print_pkt( fp, pkt )
FILE       *fp;
MPID_PKT_T *pkt;
{
    DSECTION("MPID_MPX_Get_print_pkt");
    DSECTENTRYPOINT;
...
int MPID_MPX_Print_mode( fp, pkt )
FILE        *fp;
MPID_PKT_T  *pkt;
{
    DSECTION("MPID_MPX_Print_mode");
    char *modename=0;
...
void MPID_MPX_Print_pkt_data( msg, address, len )
char *msg;
char *address;
int  len;
{
    DSECTION("MPID_MPX_Print_pkt_data");
...
void MPID_MPX_Print_Send_Handle( shandle )
MPIR_SHANDLE *shandle;
{
    DSECTION("MPID_MPX_Print_Send_Handle");
    DSECTENTRYPOINT;
 ...
 */
void MPID_MPX_Print_rhandle( fp, rhandle )
FILE *fp;
MPIR_RHANDLE *rhandle;
{
    DSECTION("MPID_MPX_Print_rhandle");
    DSECTENTRYPOINT;
...
void MPID_MPX_Print_shandle( fp, shandle )
FILE *fp;
MPIR_SHANDLE *shandle;
{
    DSECTION("MPID_MPX_Print_shandle");
    DSECTENTRYPOINT;
