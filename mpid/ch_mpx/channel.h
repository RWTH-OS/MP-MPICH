...
#define MPID_RecvAnyControl( pkt, size, from ) \
    { MPID_TRACE_CODE("BRecvAny",-1);\
      MPID_MPX_RecvAnyControl( pkt, size, from );\
...
#define MPID_RecvFromChannel( buf, size, channel ) \
    { /*struct WsockHeader h;*/\
	  ...
	  /*MPID_MPX_RecvFromChannel( &h, sizeof(h), channel );*/ \
      MPID_MPX_RecvFromChannel( buf, size, channel ); \
      MPID_TRACE_CODE("ERecvFrom",channel);}

#define MPID_ControlMsgAvail( ) \
    MPID_MPX_ControlMsgAvail( )

#define MPID_SendControl( pkt, size, channel ) \
    { MPID_TRACE_CODE("BSendControl",channel);\
      MPID_MPX_SendControl( pkt, size, channel); \
...
#define MPID_CreateRecvTransfer( buf, size, partner, id, rcvid ) \
       {*(id) = CurTag;TagsInUse++; MPID_MPX_CreateTransfer(buf,size,CurTag++,rcvid);}
...
#define MPID_TestRecvTransfer( request ) \
    MPID_MPX_TestTransfer((request)->rid )
...
#define MPID_RecvTransfer( buf, size, partner, id , recvid) {\
    ...
    MPID_MPX_ReceiveTransfer(id,recvid);\
    if (--TagsInUse == 0) CurTag = 1024; else if (id == CurTag-1) CurTag--;\
