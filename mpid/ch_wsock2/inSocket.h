...
#ifdef WSOCK_DEBUG
#define DBG(m) std::cerr<<m<<std::endl<<std::flush;
#else
#define DBG(m) 
#endif
...
{ public:
   ...
   inSocket(unsigned int port,addrType ad=any, int theType=SOCK_STREAM);
   ...
   void connect(unsigned long host,unsigned int port);
