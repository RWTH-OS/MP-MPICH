...
#define MPID_RecvAnyControl( pkt, size, from ) \
    { MPID_TRACE_CODE("BRecvAny",-1);\
      MPID_WSOCK_RecvAnyControl( pkt, size, from );\
...
#define MPID_RecvFromChannel( buf, size, channel ) \
    { /*struct WsockHeader h;*/\
	  ...
	  /*MPID_WSOCK_RecvFromChannel( &h, sizeof(h), channel );*/ \
      MPID_WSOCK_RecvFromChannel( buf, size, channel ); \
      MPID_TRACE_CODE("ERecvFrom",channel);}

#define MPID_ControlMsgAvail( ) \
    MPID_WSOCK_ControlMsgAvail( )

#define MPID_SendControl( pkt, size, channel ) \
    { MPID_TRACE_CODE("BSendControl",channel);\
      MPID_WSOCK_SendControl( pkt, size, channel); \
...
#define MPID_SendControlBlock( pkt, size, channel ) \
    { MPID_TRACE_CODE("BSendControl",channel);\
      MPID_WSOCK_send( pkt, size, channel, 1 ) \
      MPID_TRACE_CODE("ESendControl",channel);}
#endif
...
#define MPID_SendChannel( buf, size, channel ) \
    { MPID_TRACE_CODE("BSend",channel);\
      MPID_WSOCK_Send( buf, size, channel, 0 ); \
...
#define MPID_CreateRecvTransfer( buf, size, partner, id, rcvid ) \
       {*(id) = CurTag;TagsInUse++; MPID_WSOCK_CreateTransfer(buf,size,CurTag++,rcvid);}
...
#define MPID_TestRecvTransfer( request ) \
    MPID_WSOCK_TestTransfer((request)->rid )
