...
#ifndef __USOCKPACKETS_H__
#define __USOCKPACKETS_H__
...
typedef enum {	MPID_PKT_SHORT=0, MPID_PKT_LONG=1, 
				...
				MPID_PKT_SEND_ADDRESS = 5,
				MPID_PKT_REQUEST_SEND_GET=6, 
				MPID_PKT_OK_TO_SEND_GET = 7, 
				MPID_PKT_CONT_GET = 8,
				MPID_PKT_ANTI_SEND=9,
				MPID_PKT_ANTI_SEND_OK=10,
				MPID_PKT_DONE_GET = 11,
				MPID_PKT_FLOW = 13
} MPID_Pkt_t;
...
#define MPID_PKT_MODE  \
    ...
    int src_comm_lrank:32;      /* communicator relative rank of "real" source of this message */   \
...
#define MPID_PKT_MODE  \
    ...
    unsigned src_comm_lrank:11;  /* communicator relative rank of "real" source of this message */   \
    MPID_PKT_LEN_DECL               /* size of packets in bytes */        \
    MPID_PKT_LINK_DECL              /* link to 'next' packet    */        \
    MPID_PKT_SRC_DECL               /* Source of packet in COMM_WORLD system */ \
    MPID_PKT_FLOW_DECL              /* Flow control info */
...
fprintf( MPID_TRACE_FILE,"[%d] %20s on %4d (type %d) at %s:%d\n", \
	 ((MPID_USOCK_Data_global_type*)MPID_devset->active_dev->global_data)->MyWorldRank, name, channel, mode, __FILE__, __LINE__ ); \
