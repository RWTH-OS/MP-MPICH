...
/* the device number is only required when using the multi-device environment */
#ifndef DEVICE_ch_p4_nbr
#define DEVICE_ch_p4_nbr 0
/* (if the device number is not defined, we just set it to zero) */
#endif
...
#   endif
    ...
    sprintf( ch_p4_dev_nbr, "%d", DEVICE_ch_p4_nbr );
...
*/
		...
		rc = execlp(remote_shell, remote_shell,
			    ...
			    "-p4yourname", host, "-p4rmrank", rm_rank_str,
#endif
			    /* we need to supply the remote processes with the
			     device number, otherwise they won't use ch_p4, unless
			    it is the default device (maybe an alternative solution to
			    this problem would be to start all remote processes manually,
			    then it would not be necessary to supply this cmdline-option
			    here)*/
			    "-usedevice ", ch_p4_dev_nbr,
			    NULL);
