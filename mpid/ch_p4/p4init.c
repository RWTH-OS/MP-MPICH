...
static MPID_CH_P4_long_len_value;
static MPID_CH_P4_vlong_len_value;
...
/* Forward refs */
int MPID_CH_P4_End ANSI_ARGS(( MPID_Device * ));
int MPID_CH_P4_Abort ANSI_ARGS(( struct MPIR_COMMUNICATOR *, int, char * ));
void MPID_CH_P4_Version_name ANSI_ARGS(( char * ));

int MPID_CH_P4_long_len( int );
int MPID_CH_P4_vlong_len( int );
...
MPID_Device *MPID_CH_P4_InitMsgPass( argc, argv, short_len, long_len )
int  *argc;
char ***argv;
int  short_len, long_len;
{
    ...
    MPID_CH_P4_long_len_value = short_len;
    dev->long_len     = &MPID_CH_P4_long_len;
    MPID_CH_P4_vlong_len_value = long_len;
    dev->vlong_len    = &MPID_CH_P4_vlong_len;
    ...
    dev->terminate    = MPID_CH_P4_End;
    dev->abort	      = MPID_CH_P4_Abort;
    dev->next	      = 0;
...
int MPID_CH_P4_End( 
	MPID_Device *dev )
{
    DEBUG_PRINT_MSG("Entering MPID_CH_P4_End\n");
    ...
    /* MPID_Close_sockets(); */
    DEBUG_PRINT_MSG("Leaving MPID_CH_P4_End");
    return 0;
}

void MPID_CH_P4_Version_name( char *name )
{
    sprintf( name, "ADI version %4.2f - transport %s", MPIDPATCHLEVEL, 
	     MPIDTRANSPORT );
}

int MPID_CH_P4_long_len( int partner_devlrank )
{
    return MPID_CH_P4_long_len_value;
}

int MPID_CH_P4_vlong_len( int partner_devlrank )
{
    return MPID_CH_P4_vlong_len_value;
}
