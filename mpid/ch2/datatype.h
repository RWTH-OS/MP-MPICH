...
/* Internal Datatypes for the stack */

typedef struct {
    int   extent; /* extent of one item of this level (in bytes)*/
    int     size; /* size of !1! item of this level (in bytes) */
    int    count; /* replication count */
} MPIR_FF_STACK_ITEM;

typedef struct FF_STACK_T MPIR_FF_LIST_ITEM;

struct FF_STACK_T {
    MPIR_FF_STACK_ITEM *stack; /* stack for the leave (array) */
    int           contig_size; /* contiguous size of the leave (in bytes) */
    int                   top; /* top of the stack */
    int                   pos; /* offset to start in source buffer (in bytes) */
    MPIR_FF_LIST_ITEM    *next;
};
...
struct MPIR_DATATYPE {
    ...
    int		*known; /* whether the datatype is already known on 
			   remote node (array, one for each node) */
    ...
    int              depth; /* depth of the datatype tree, used for the
			       flattening on the fly algorithm */
    int             leaves; /* number of leaves in this datatype */
    MPIR_FF_LIST_ITEM  *ff; /* list of stacks for the leaves */

    unsigned int    flatcount;
    MPIR_FLAT_BUF  *flattened; /* Used to flatten the data type*/
...
#define MPIR_TEST_DTYPE(idx,ptr,comm,routine_name) {\
   if (!(ptr)) {RETURNV(MPIR_ERROR(comm,MPIR_ERRCLASS_TO_CODE(MPI_ERR_TYPE,MPIR_ERR_TYPE_NULL),routine_name));/*Errorcode 323*/}\
