...
MPID_Msg_pack_t *msgact;
{
	...
	*msgact = MPID_MSG_OK;
	...
	if (MPID_procinfo[MPID_MyAllRank].byte_order == MPID_H_XDR ||
	    MPID_procinfo[partner].byte_order == MPID_H_XDR) {
	    *msgrep = MPID_MSGREP_XDR;
	    *msgact = MPID_MSG_XDR;
	}
	else if (MPID_procinfo[MPID_MyAllRank].byte_order != 
...
void MPID_Msg_act( msgrep, msgact )
MPID_Msgrep_t   msgrep;
MPID_Msg_pack_t *msgact;
{
    int mpi_errno;
...
void            *src, *dest;
int             count, maxcount, *position, *error_code;
MPID_Msgrep_t   msgrep;
struct MPIR_DATATYPE *dtype_ptr;
MPID_Msg_pack_t msgact;
{
    int (*packcontig) (unsigned char *, unsigned char *, struct MPIR_DATATYPE *, int, void * ) = 0;
    void *packctx = 0;
    int  outlen;
    int  err;
#ifdef HAS_XDR
...
void MPID_Unpack( src, maxcount, msgrep, in_position, 
		  ...
		  comm_ptr, error_code )
void          *src, *dest;
int           maxcount, *in_position, count, *out_position, 
              *error_code;
struct MPIR_DATATYPE     *dtype_ptr;
struct MPIR_COMMUNICATOR *comm_ptr;
MPID_Msgrep_t msgrep;
