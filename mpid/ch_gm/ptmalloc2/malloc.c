 ...
 *   - registration makes releasing memory to the system more costly, 
 *     so changes some configuration parameters accordingly (MMAP THRESHOLD).
...
#define munmap(a,b) gmpi_munmap(a,b)
...
* Version ptmalloc2-20011215
  $Id: malloc.c,v 1.1.4.1 2004/12/08 17:17:32 carsten Exp $
  based on:
...
#define bin_at(m, i) \
  ((mbinptr) \
    (((char*)(&((m)->mu.ref_bins))) + ((i)*(sizeof(mchunkptr)<<1)) - (SIZE_SZ<<1)) \
  )
