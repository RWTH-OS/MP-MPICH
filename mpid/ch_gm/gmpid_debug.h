...
#ifndef _GMPID_DEBUG_H
#define _GMPID_DEBUG_H
#include <stdio.h>
#include "gmpi_packets.h"

int  MPID_GM_Print_mode   ( FILE *, MPID_PKT_T * );
int  MPID_GM_Print_packet ( FILE *, MPID_PKT_T * );
void MPID_GM_Print_pkt_data ( char *, char *, int );
void MPID_GM_SetMsgDebugFlag (int);
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, \
	      "[%d]R About to copy to %lx from %lx (n=%d) (%s:%d)...\n", \
           MPID_GM_rank, (long)a, (long)b, c, __FILE__, __LINE__ );\
     MPID_DEBUG_MSG;\
     MPID_SAVE_MSG;}\
memcpy( a, b, c );}
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, \
           ...
           MPID_GM_rank, msg, tag, src_lrank, context_id, \
            __FILE__, __LINE__ );\
     MPID_DEBUG_MSG;\
     MPID_SAVE_MSG;}}
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, \
	   ...
	   MPID_GM_rank, msg, (pkt)->tag, dest, \
           (pkt)->context_id, (pkt)->len );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       MPID_GM_Print_mode( MPID_DEBUG_FILE, (MPID_PKT_T *)(pkt) );\
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, \
              "[%d]%s of tag = %d, dest = %d, ctx = %d, len = %d, mode = ", \
           MPID_GM_rank, msg, (pkt)->tag, destination, \
           (pkt)->context_id, (pkt)->len );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf,\
	      "[%d]%s ", MPID_GM_rank, msg );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       MPID_GM_Print_packet( MPID_DEBUG_FILE, (MPID_PKT_T *)(pkt) );\
       FPRINTF( MPID_DEBUG_FILE, "(%s:%d)\n", __FILE__, __LINE__ );\
       fflush( MPID_DEBUG_FILE );}\
     MPID_SAVE_MSG;}}
...
{  if (MPID_DebugFlag) {\
     if (MPID_UseDebugFile) {\
       FPRINTF( MPID_DEBUG_FILE,\
              "[%d]%s of tag = %d, dest = %d, ctx = %d, len = %d, mode = ", \
	     MPID_GM_rank, msg, (pkt)->tag, dest, (pkt)->context_id, \
	     (pkt)->len );\
       MPID_GM_Print_mode( MPID_DEBUG_FILE, (MPID_PKT_T *)(pkt) );\
       FPRINTF( MPID_DEBUG_FILE, "(%s:%d)\n", __FILE__, __LINE__ );\
       MPID_GM_Print_packet( MPID_DEBUG_FILE, (MPID_PKT_T *)(pkt) );\
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf, "[%d]%s (%s:%d)\n", \
           MPID_GM_rank, msg, __FILE__, __LINE__ );\
     MPID_DEBUG_MSG;\
     MPID_SAVE_MSG;}}
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf,\
	    ...
	    MPID_GM_rank, msg, (pkt)->head.tag, from_grank, \
...
{  if (MPID_DebugFlag) {\
     SPRINTF( ch_debug_buf,\
            "[%d]%s for tag = %d, source = %d, ctx = %d, len = %d, mode = ", \
	    MPID_GM_rank, msg, (pkt)->head.tag, from, \
	    (pkt)->head.context_id, (pkt)->head.len );\
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       SMPI_Print_mode( MPID_DEBUG_FILE, (SMPI_PKT_T *)(pkt) );\
...
"[%d]%s for tag = %d, source = %d, ctx = %d, len = %d, mode = ", \
	  ...
	  MPID_GM_Print_mode( MPID_DEBUG_FILE, (MPID_PKT_T *)(pkt) );\
	  FPRINTF( MPID_DEBUG_FILE, "(%s:%d)\n", __FILE__, __LINE__ );\
	  MPID_GM_Print_packet( MPID_DEBUG_FILE,\
		             (MPID_PKT_T *)(pkt) );\
	   fflush( MPID_DEBUG_FILE );}\
	}}
...
{  if (MPID_DebugFlag) {\
     ...
     if (MPID_UseDebugFile) {\
       fputs( ch_debug_buf, MPID_DEBUG_FILE );\
       MPID_GM_Print_packet( MPID_DEBUG_FILE, (pkt) );\
       fflush( MPID_DEBUG_FILE );}\
     MPID_SAVE_MSG;}}	
...
if (MPID_DebugFlag) {\
    if (MPID_UseDebugFile) {\
      FPRINTF( MPID_DEBUG_FILE, \
	     ...
	     MPID_GM_rank, *(int *)mpid_send_handle->start, \
	     __FILE__, __LINE__ );\
