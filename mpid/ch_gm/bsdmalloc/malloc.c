...
#define brk(n)       gmpi_brk(n)
#define sbrk(n)      gmpi_sbrk(n)
#define munmap(p,l)  gmpi_munmap(p,l)
...
#ifndef ZEROSIZEPTR
#define ZEROSIZEPTR	((void *)(gm_up_t)(1 << (malloc_pageshift - 1)))
#endif
...
wrterror(char *p)
{
    ...
    suicide = 1;
    wrtmessage("", malloc_func, " error: ", p);
    abort();
}
...
wrtwarning(char *p)
{
    ...
    if (malloc_abort)
	wrterror(p);
    wrtmessage("", malloc_func, " warning: ", p);
}
...
imalloc(size_t size)
{
	...
	result = 0;
    else if ((size + malloc_pagesize) >= (gm_up_t)page_dir)
	result = 0;
