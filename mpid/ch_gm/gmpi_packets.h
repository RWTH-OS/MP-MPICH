...
#ifndef _GMPI_PACKETS_H
#define _GMPI_PACKETS_H
#include <stdio.h>
#include "mpid.h"
/* 
...
/* rndv done, needed by ch_gm */
typedef struct {
    MPID_PKT_MODE
    MPID_Aint recv_id;         /* Id needed in case of a cancel */
    MPID_Aint done_target_ptr;
} MPID_PKT_DONE_T;
...
typedef struct {
    ...
    MPID_Aint target_ptr;       /* needed by ch_gm */
...
typedef union _MPID_PKT_T {
    ...
    MPID_PKT_DONE_T          done_pkt; /* needed by ch_gm */
...
fprintf( MPID_TRACE_FILE,"[%d] %20s on %4d (type %d) at %s:%d\n", \
	 MPID_GM_rank, name, channel, mode, __FILE__, __LINE__ ); \
