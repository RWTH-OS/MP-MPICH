...
struct MPIR_DATATYPE *dtypeptr;
{
    ...
    if(dtypeptr) {
	/* This message has to be packed before sending*/
	len *= dtypeptr->size;
	shandle->curr_buf = buf;
	shandle->act_index=0;
	shandle->datatype = dtypeptr;
	MPIR_REF_INCR(dtypeptr);
    } else {
	shandle->curr_buf = 0;
	shandle->datatype = 0;
    }
    ...
    pkt->mode	    = MPID_PKT_REQUEST_SEND_GETSH;
    ...
    MPID_n_pending++;
    MPID_SHMEM_SendControl( (MPID_PKT_TSH*)pkt, sizeof(MPID_PKT_GET_TSH), dest );
...
int   from_grank;
{
    MPID_PKT_GET_TSH spkt;
    MPID_PKT_GET_TSH *pkt = &spkt;
    int            len;
    int            is_done;
    
    MEMCPY( pkt, (MPID_PKT_GET_TSH *)in_pkt, sizeof(MPID_PKT_GET_TSH) );
    MPID_PKT_READY_CLR(&(((MPID_PKT_GET_TSH*)in_pkt)->ready));
    
    if (pkt->mode == MPID_PKT_OK_TO_SEND_GETSH) {
	...
	if (pkt->len_avail == 0) {
	    ...
	    /* This results in one more message but simplifies the 
	       rest of the code 
	    */
	    /* Here we check the length the receiver wants to get.
	       This is a simple way to check for truncation.
	    */
	    if(pkt->len < shandle->bytes_as_contig)
		shandle->bytes_as_contig = pkt->len;

	    shandle->rndv_buf_len = shandle->bytes_as_contig;
	    /*pkt->len_avail = shandle->bytes_as_contig;*/
	    if (shandle->rndv_buf_len > 0) {
		pkt->address = MPID_SetupGetAddress( shandle->start, 
		    &shandle->rndv_buf_len, from_grank );
	    } else pkt->address = 0;
	    pkt->len_avail = shandle->rndv_buf_len;
	    pkt->cur_offset = 0;
	} else {
	...
	if (len > shandle->rndv_buf_len) {
	    len = shandle->rndv_buf_len;
	    is_done = 0;
	} else {
	    is_done = 1;
	    pkt->len_avail = len;
	...
	if (len > 0) {
	    if(shandle->curr_buf) {
		/* we have to pack the message into the send buffer*/	    
		pkt->len_avail= MPIR_Pack_flat_restricted(pkt->address,&shandle->curr_buf,&shandle->act_index,
		                                          len,shandle->datatype);
		is_done = (is_done && (pkt->len_avail == len));
	    } else {
		MEMCPY( pkt->address, ((char *)shandle->start) + pkt->cur_offset,len );
	    }
	    DEBUG_PRINT_BASIC_SEND_PKT("S Sending cont-get message",pkt);
	    MPID_SHMEM_SendControl( (MPID_PKT_TSH*)pkt, sizeof(MPID_PKT_GET_TSH), from_grank );	
	}
	...
	if (is_done) {
	    ...
	    /* printf( "Completed rendezvous send shandle = %p (ref=%d)\n", 
	    ...
	    if(shandle->datatype) MPIR_Type_free(&shandle->datatype);
	}
    } else if (pkt->mode == MPID_PKT_CONT_GETSH) {
	/* Data is available */
	...
	/* We have all the data; the transfer is complete and
	    ...
	    pkt->mode = MPID_PKT_OK_TO_SEND_GETSH;
...
/*
 * This is the routine called when a packet of type MPID_PKT_REQUEST_SEND_GET is
...
void         *in_pkt;
{
    MPID_PKT_GET_TSH spkt, *pkt = &spkt;
...
MPID_SHMEM_Rndvn_cancel_recv(MPIR_RHANDLE*);
 
MPID_SHMEM_Rndvn_cancel_recv(runex) 
MPIR_RHANDLE *runex;
{
...
void         *in_pkt;
{
    MPID_PKT_GET_TSH   *pkt = (MPID_PKT_GET_TSH *)in_pkt;
...
void         *in_runex;
{
    ...
    /* Send back an "ok to proceed" packet */
    MPID_CHK_MSGLEN(rhandle,rhandle->s.count,err)
    pkt->len	    = rhandle->s.count;
    pkt->mode	    = MPID_PKT_OK_TO_SEND_GETSH;
    pkt->cur_offset = 0;
    pkt->len_avail  = 0;
    pkt->address    = 0;
    ...
    DEBUG_PRINT_BASIC_SEND_PKT("R Sending ok-to-send message",pkt);
    MPID_SHMEM_SendControl( (MPID_PKT_TSH*)pkt, sizeof(MPID_PKT_GET_TSH), runex->from );
