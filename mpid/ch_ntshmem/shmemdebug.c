...
void MPID_Get_print_pkt ANSI_ARGS(( FILE *, MPID_PKT_TSH *));
int MPID_Cancel_print_pkt ANSI_ARGS(( FILE *, MPID_PKT_TSH *));
int  MPID_Rndv_print_pkt ANSI_ARGS((FILE *, MPID_PKT_TSH *));
...
MPID_PKT_TSH *pkt;
{
    ...
    if (pkt->head.mode != MPID_PKT_LONGSH) {
	/* begin if mode != address */
	send_id = pkt->get_pkt.send_id;
	if (pkt->head.mode != MPID_PKT_REQUEST_SEND_GETSH)
	    recv_id = pkt->get_pkt.recv_id;

	sprintf( sendid, "%p", send_id );
	if (pkt->head.mode != MPID_PKT_REQUEST_SEND_GETSH)
	    sprintf( recvid, "%p", recv_id );
    }  /* end if mode != address */
...
MPID_PKT_TSH  *pkt;
{
	...
	FPUTS( "long", fp );
	break;
    case MPID_PKT_REQUEST_SEND_GETSH:
	FPUTS( "do get", fp );
	break; 
...
int  len;
{
    ...
    if (len < 78 && address) {
	for (i=0; i<len; i++) {
	    FPRINTF( MPID_DEBUG_FILE, "%p", aa[i] );
	}
